...
func (st *State) addStorageForUnit(
	ctx context.Context, tx *sqlair.TX, unitUUID coreunit.UUID, storageArg internal.AddUnitStorageArg,
) ([]string, error) {
...
func (st *State) AddStorageForCAASUnit(
	...
	storageArg internal.AddUnitStorageArg,
...
func (st *State) AddStorageForIAASUnit(
	ctx context.Context, unitUUID coreunit.UUID,
	storageArg internal.AddIAASUnitStorageArg,
) ([]corestorage.ID, error) {
	db, err := st.DB(ctx)
	if err != nil {
		return nil, errors.Capture(err)
	...
	var storageIDs []string
	err = db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		storageIDs, err = st.addStorageForUnit(ctx, tx, unitUUID, storageArg.AddUnitStorageArg)
		if err != nil {
			return errors.Capture(err)
		}
