...
func (u *unitStorageSuite) newUnitWithStorageDirectives(c *tc.C) (coreunit.UUID, domainstorage.StoragePoolUUID) {
	...
	_, err := u.DB().ExecContext(
		c.Context(),
		"SELECT  unit_storage_directive VALUES (?, ?, ?, ?, ?, ?)",
		unitUUID.String(),
		charmUUID,
		"st1",
		storagePoolUUID.String(),
...
func (u *unitStorageSuite) TestAddStorageForIAASUnitNotFound(c *tc.C) {
	...
	_, err := u.state.AddStorageForIAASUnit(c.Context(), uuid, internal.AddIAASUnitStorageArg{})
...
func (u *unitStorageSuite) TestAddStorageForIAASUnit(c *tc.C) {
	...
	gotIDs, err := u.state.AddStorageForIAASUnit(c.Context(), unitUUID, internal.AddIAASUnitStorageArg{
		AddUnitStorageArg: internal.AddUnitStorageArg{
