...
// MakeAddUnitStorageArgs creates the storage arguments required to
...
func (s Service) MakeAddUnitStorageArgs(
	ctx context.Context,
	unitUUID coreunit.UUID,
	sd application.StorageDirective,
) (internal.AddUnitStorageArg, error) {
	ctx, span := trace.Start(ctx, trace.NameFromFunc())
	defer span.End()
	...
	if err != nil {
		return internal.AddUnitStorageArg{}, errors.Errorf(
			"making new storage %q instance args: %w", sd.Name, err,
		)
	}
	...
	if err != nil {
		return internal.AddUnitStorageArg{}, errors.Errorf("getting unit net node uuid: %w", err)
	}
	...
	return internal.AddUnitStorageArg{
		StorageInstances: rvalInstances,
		StorageToAttach:  rvalToAttach,
		StorageToOwn:     rvalToOwn,
	}, nil
