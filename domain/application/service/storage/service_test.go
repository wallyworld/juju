...
func (s *serviceSuite) TestMakeAddUnitStorageArgs(c *tc.C) {
	ctrl := s.setupMocks(c)
	defer ctrl.Finish()
	...
	s.state.EXPECT().GetUnitNetNode(gomock.Any(), unitUUID).Return(attachNetNodeUUID.String(), nil)

	svc := NewService(s.state, s.poolProvider, loggertesting.WrapCheckLog(c))

	arg, err := svc.MakeAddUnitStorageArgs(
		c.Context(),
		unitUUID,
		storageDirective,
	)
	...
	c.Check(arg, createUnitStorageArgChecker(), internal.AddUnitStorageArg{
		StorageInstances: expectedStorageInstances,
		StorageToAttach:  expectedStorageToAttach,
		StorageToOwn:     expectedStorageToOwn,
	})
