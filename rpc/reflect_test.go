...
func TestReflectSuite(t *tctesting.T) {
...
func (*reflectSuite) TestFindMethod(c *tc.C) {
	...
	c.Assert(err, tc.ErrorMatches, `unknown object type "foo"`)
	c.Assert(err, tc.FitsTypeOf, (*rpcreflect.CallNotImplementedError)(nil))
	c.Assert(m, tc.IsNil)

	m, err = v.FindMethod("SimpleMethods", 0, "bar")
	c.Assert(err, tc.ErrorMatches, "no such request - method SimpleMethods.bar is not implemented")
	c.Assert(err, tc.FitsTypeOf, (*rpcreflect.CallNotImplementedError)(nil))
	c.Assert(m, tc.IsNil)
