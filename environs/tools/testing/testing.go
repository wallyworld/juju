...
func makeTools(c tc.LikeC, metadataDir, stream string, versionStrings []string, withCheckSum bool) coretools.List {
	...
	for _, versionString := range versionStrings {
		...
		if err != nil {
			c.Assert(err, tc.Satisfies, corebase.IsUnknownOSForSeriesError)
		}
...
func UploadToStorage(c *tc.C, stor storage.Storage, stream string, versions ...version.Binary) map[version.Binary]string {
...
func UploadToDirectory(c *tc.C, dir string, streamVersions StreamVersions) map[string]map[version.Binary]string {
