...
func (s *bootstrapSuite) TestBootstrapSpecifiedBootstrapSeries(c *tc.C) {
	env := newEnviron("foo", useDefaultKeys, nil)
	s.setDummyStorage(c, env)
	...
	c.Check(env.args.BootstrapBase, tc.Equals, corebase.MustParseBaseFromString("ubuntu@22.04"))
	c.Check(env.args.AvailableTools.AllReleases(), tc.SameContents, []string{"ubuntu"})
}

func (s *bootstrapSuite) TestBootstrapFallbackBootstrapSeries(c *tc.C) {
	env := newEnviron("foo", useDefaultKeys, nil)
	s.setDummyStorage(c, env)
	cfg, err := env.Config().Apply(map[string]interface{}{
...
func (s *bootstrapSuite) TestBootstrapForcedBootstrapSeries(c *tc.C) {
	env := newEnviron("foo", useDefaultKeys, nil)
	s.setDummyStorage(c, env)
	cfg, err := env.Config().Apply(map[string]interface{}{
	...
	c.Assert(err, tc.ErrorIsNil)
	c.Check(env.bootstrapCount, tc.Equals, 1)
	c.Check(env.args.BootstrapBase, tc.Equals, corebase.MustParseBaseFromString("ubuntu@20.04"))
	c.Check(env.args.AvailableTools.AllReleases(), tc.SameContents, []string{"ubuntu"})
}
...
func (s *bootstrapSuite) TestBootstrapWithInvalidBootstrapSeries(c *tc.C) {
	env := newEnviron("foo", useDefaultKeys, nil)
	s.setDummyStorage(c, env)
	cfg, err := env.Config().Apply(map[string]interface{}{
		"default-series": "jammy",
...
func (s *bootstrapSuite) TestBootstrapControllerCharmLocal(c *tc.C) {
	...
	err := bootstrap.Bootstrap(modelcmd.BootstrapContext(c.Context(), ctx), env,
		s.callContext, bootstrap.BootstrapParams{
			ControllerConfig:        coretesting.FakeControllerConfig(),
			AdminSecret:             "admin-secret",
			CAPrivateKey:            coretesting.CAKey,
...
func (s *bootstrapSuite) setupBootstrapSpecificVersion(c *tc.C, clientMajor, clientMinor int, toolsVersion *version.Number) (error, int, version.Number) {
...
func (s *bootstrapSuite) TestBootstrapSpecificVersion(c *tc.C) {
	...
	c.Assert(vers, tc.DeepEquals, version.Number{
		Major: 10,
		Minor: 11,
		Patch: 12,
	})
...
func TestBootstrapContextSuite(t *tctesting.T) {
