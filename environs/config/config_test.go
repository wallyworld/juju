...
func (test configTest) check(c *tc.C) {
	...
	if s := test.attrs["agent-version"]; s != nil {
		...
		c.Check(agentVersion, tc.Equals, version.MustParse(s.(string)))
	} else {
		c.Check(ok, tc.IsFalse)
		c.Check(agentVersion, tc.Equals, version.Zero)
	}
	...
	c.Check(cfg.AuthorizedKeys(), tc.Equals, keys)

	lfCfg, hasLogCfg := cfg.LogFwdSyslog()
	if v, ok := test.attrs["logforward-enabled"].(bool); ok {
		if c.Check(hasLogCfg, tc.IsTrue) {
			c.Check(lfCfg.Enabled, tc.Equals, v)
		}
	}
	if v, ok := test.attrs["syslog-ca-cert"].(string); v != "" {
		c.Check(hasLogCfg, tc.IsTrue)
		c.Check(lfCfg.CACert, tc.Equals, v)
	} else if ok {
		c.Check(hasLogCfg, tc.IsTrue)
		c.Check(lfCfg.CACert, tc.Equals, "")
	}
	if v, ok := test.attrs["syslog-client-cert"].(string); v != "" {
		c.Check(hasLogCfg, tc.IsTrue)
		c.Check(lfCfg.ClientCert, tc.Equals, v)
	} else if ok {
		c.Check(hasLogCfg, tc.IsTrue)
		c.Check(lfCfg.ClientCert, tc.Equals, "")
	}
	if v, ok := test.attrs["syslog-client-key"].(string); v != "" {
		c.Check(hasLogCfg, tc.IsTrue)
		c.Check(lfCfg.ClientKey, tc.Equals, v)
	} else if ok {
		c.Check(hasLogCfg, tc.IsTrue)
		c.Check(lfCfg.ClientKey, tc.Equals, "")
	}
	...
	c.Assert(agentStreamValue, tc.Equals, expectedAgentStreamAttr)
...
func (s *ConfigSuite) TestConfigAttrs(c *tc.C) {
...
func (s *ConfigSuite) TestLoggingOutput(c *tc.C) {
	config := newTestConfig(c, testing.Attrs{})
	loggingOutput, ok := config.LoggingOutput()
	c.Assert(ok, tc.IsFalse)
	c.Assert(loggingOutput, tc.DeepEquals, []string{})
	...
	loggingOutput, ok = config.LoggingOutput()
	c.Assert(ok, tc.IsTrue)
	c.Assert(loggingOutput, tc.DeepEquals, []string{"database", "syslog"})
	...
	loggingOutput, ok = config.LoggingOutput()
	c.Assert(ok, tc.IsTrue)
	c.Assert(loggingOutput, tc.DeepEquals, []string{"syslog"})
}
...
func (s *ConfigSuite) TestStatusHistoryConfigDefaults(c *tc.C) {
	cfg := newTestConfig(c, testing.Attrs{})
	c.Assert(cfg.MaxStatusHistoryAge(), tc.Equals, 336*time.Hour)
	c.Assert(cfg.MaxStatusHistorySizeMB(), tc.Equals, uint(5120))
}

func (s *ConfigSuite) TestStatusHistoryConfigValues(c *tc.C) {
	cfg := newTestConfig(c, testing.Attrs{
		"max-status-history-size": "8G",
		"max-status-history-age":  "96h",
	})
	c.Assert(cfg.MaxStatusHistoryAge(), tc.Equals, 96*time.Hour)
	c.Assert(cfg.MaxStatusHistorySizeMB(), tc.Equals, uint(8192))
}
