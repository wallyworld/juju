...
func TestVolumeStateSuite(t *tctesting.T) {
	testing.MgoTestPackage(t, &VolumeStateSuite{})
}

func (s *VolumeStateSuite) TestAddMachine(c *tc.C) {
...
func (s *VolumeStateSuite) TestAssignToMachine(c *tc.C) {
	_, unit, _ := s.setupSingleStorage(c, "block", "loop-pool")
...
func (s *VolumeStateSuite) assertMachineVolume(c *tc.C, unit *state.Unit) {
	...
	c.Assert(volume.VolumeTag(), tc.Equals, names.NewVolumeTag("0/0"))
	...
	c.Assert(volumeAttachments[0].Volume(), tc.Equals, volume.VolumeTag())
	c.Assert(volumeAttachments[0].Host(), tc.Equals, machine.MachineTag())
...
func (s *VolumeStateSuite) TestAddApplicationInvalidPool(c *tc.C) {
...
func (s *VolumeStateSuite) TestAddApplicationNoUserDefaultPool(c *tc.C) {
	ch := s.AddTestingCharm(c, "storage-block")
	testStorage := map[string]state.StorageConstraints{
...
func (s *VolumeStateSuite) TestAddApplicationDefaultPool(c *tc.C) {
...
func (s *VolumeStateSuite) TestSetVolumeInfo(c *tc.C) {
...
func (s *VolumeStateSuite) TestSetVolumeInfoNoVolumeId(c *tc.C) {
	_, u, storageTag := s.setupSingleStorage(c, "block", "loop-pool")
	err := s.State.AssignUnit(u, state.AssignCleanEmpty)
	c.Assert(err, tc.ErrorIsNil)
	...
	err = s.storageBackend.SetVolumeInfo(volume.VolumeTag(), volumeInfoSet)
	c.Assert(err, tc.ErrorMatches, `cannot set info for volume "0/0": volume ID not set`)
}

func (s *VolumeStateSuite) TestSetVolumeInfoNoStorageAssigned(c *tc.C) {
...
func (s *VolumeStateSuite) TestSetVolumeInfoImmutable(c *tc.C) {
	_, u, storageTag := s.setupSingleStorage(c, "block", "loop-pool")
	err := s.State.AssignUnit(u, state.AssignCleanEmpty)
	c.Assert(err, tc.ErrorIsNil)
	volume := s.storageInstanceVolume(c, storageTag)
	...
	err = s.storageBackend.SetVolumeInfo(volume.VolumeTag(), volumeInfoSet)
	c.Assert(err, tc.ErrorMatches, `cannot set info for volume "0/0": cannot change pool from "loop-pool" to ""`)

	volumeInfoSet.Pool = "loop-pool"
	s.assertVolumeInfo(c, volumeTag, volumeInfoSet)
}

func (s *VolumeStateSuite) TestWatchVolumeAttachment(c *tc.C) {
	_, u, storageTag := s.setupSingleStorage(c, "block", "loop-pool")
	err := s.State.AssignUnit(u, state.AssignCleanEmpty)
	c.Assert(err, tc.ErrorIsNil)
...
func (s *VolumeStateSuite) TestWatchModelVolumes(c *tc.C) {
...
func (s *VolumeStateSuite) TestWatchModelVolumeAttachments(c *tc.C) {
	app := s.setupMixedScopeStorageApplication(c, "block")
	addUnit := func() {
		u, err := app.AddUnit(state.AddUnitParams{})
		c.Assert(err, tc.ErrorIsNil)
...
func (s *VolumeStateSuite) TestWatchMachineVolumes(c *tc.C) {
...
func (s *VolumeStateSuite) TestWatchMachineVolumeAttachments(c *tc.C) {
	app := s.setupMixedScopeStorageApplication(c, "block", "machinescoped", "modelscoped")
...
func (s *VolumeStateSuite) TestParseVolumeAttachmentId(c *tc.C) {
	assertValid := func(id string, m names.Tag, v names.VolumeTag) {
		hostTag, volumeTag, err := state.ParseVolumeAttachmentId(id)
		c.Assert(err, tc.ErrorIsNil)
		c.Assert(hostTag, tc.Equals, m)
		c.Assert(volumeTag, tc.Equals, v)
	}
...
func (s *VolumeStateSuite) TestParseVolumeAttachmentIdError(c *tc.C) {
...
func (s *VolumeStateSuite) TestAllVolumes(c *tc.C) {
	...
	c.Assert(tags, tc.SameContents, expected)
}

func (s *VolumeStateSuite) assertCreateVolumes(c *tc.C) (_ *state.Machine, all, persistent []names.VolumeTag) {
...
func (s *VolumeStateSuite) TestRemoveStorageInstanceDestroysAndUnassignsVolume(c *tc.C) {
	...
	c.Assert(v.Life(), tc.Equals, state.Dying)
}

func (s *VolumeStateSuite) TestReleaseStorageInstanceVolumeReleasing(c *tc.C) {
	_, u, storageTag := s.setupSingleStorage(c, "block", "modelscoped")
	err := s.State.AssignUnit(u, state.AssignCleanEmpty)
	c.Assert(err, tc.ErrorIsNil)
	volume := s.storageInstanceVolume(c, storageTag)
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(volume.Releasing(), tc.IsFalse)
	err = s.storageBackend.SetVolumeInfo(volume.VolumeTag(), state.VolumeInfo{VolumeId: "vol-123"})
	c.Assert(err, tc.ErrorIsNil)
	...
	c.Assert(volume.Releasing(), tc.IsTrue)
}

func (s *VolumeStateSuite) TestReleaseStorageInstanceVolumeUnreleasable(c *tc.C) {
...
func (s *VolumeStateSuite) TestSetVolumeAttachmentInfoVolumeNotProvisioned(c *tc.C) {
	_, u, storageTag := s.setupSingleStorage(c, "block", "loop-pool")
	err := s.State.AssignUnit(u, state.AssignCleanEmpty)
	c.Assert(err, tc.ErrorIsNil)
	assignedMachineId, err := u.AssignedMachineId()
	...
	c.Assert(err, tc.ErrorMatches, `cannot set info for volume attachment 0/0:0: volume "0/0" not provisioned`)
}

func (s *VolumeStateSuite) TestDestroyVolume(c *tc.C) {
...
func (s *VolumeStateSuite) TestDestroyVolumeNotFound(c *tc.C) {
	err := s.storageBackend.DestroyVolume(names.NewVolumeTag("0"), false)
	c.Assert(err, tc.ErrorMatches, `destroying volume 0: volume "0" not found`)
	c.Assert(err, tc.Satisfies, errors.IsNotFound)
}

func (s *VolumeStateSuite) TestDestroyVolumeStorageAssigned(c *tc.C) {
	volume, _, u := s.setupStorageVolumeAttachment(c)
	storageTag, err := volume.StorageInstance()
	c.Assert(err, tc.ErrorIsNil)

	err = s.storageBackend.DestroyVolume(volume.VolumeTag(), false)
	c.Assert(err, tc.ErrorMatches, "destroying volume 0: volume is assigned to storage data/0")
...
func (s *VolumeStateSuite) TestDestroyVolumeNoAttachments(c *tc.C) {
...
func (s *VolumeStateSuite) TestDetachVolumeDyingAttachment(c *tc.C) {
	volume, machine := s.setupModelScopedVolumeAttachment(c)
	volumeTag := volume.VolumeTag()
...
func (s *VolumeStateSuite) TestDetachVolumeDyingAttachmentPlan(c *tc.C) {
	volume, machine := s.setupModelScopedVolumeAttachment(c)
	machineTag := machine.MachineTag()
	volumeTag := volume.VolumeTag()
	err := machine.SetProvisioned("inst-id", "", "fake_nonce", nil)
	...
	volumeAttachment := s.volumeAttachment(c, machineTag, volumeTag)
	c.Assert(volumeAttachment.Life(), tc.Equals, state.Alive)
	volumeAttachmentPlan := s.volumeAttachmentPlan(c, machineTag, volumeTag)
	c.Assert(volumeAttachmentPlan.Life(), tc.Equals, state.Dying)
	volume = s.volume(c, volumeTag)
...
func (s *VolumeStateSuite) TestRemoveVolume(c *tc.C) {
...
func (s *VolumeStateSuite) TestRemoveVolumeNotFound(c *tc.C) {
	err := s.storageBackend.RemoveVolume(names.NewVolumeTag("42"))
	c.Assert(err, tc.ErrorIsNil)
}

func (s *VolumeStateSuite) TestRemoveVolumeNotDead(c *tc.C) {
	volume, _ := s.setupMachineScopedVolumeAttachment(c)
	err := s.storageBackend.RemoveVolume(volume.VolumeTag())
	c.Assert(err, tc.ErrorMatches, "removing volume 0/0: volume is not dead")
	err = s.storageBackend.DestroyVolume(volume.VolumeTag(), false)
	c.Assert(err, tc.ErrorIsNil)
	err = s.storageBackend.RemoveVolume(volume.VolumeTag())
	c.Assert(err, tc.ErrorMatches, "removing volume 0/0: volume is not dead")
...
func (s *VolumeStateSuite) TestDetachVolume(c *tc.C) {
	volume, machine := s.setupModelScopedVolumeAttachment(c)
	assertDetach := func() {
		err := s.storageBackend.DetachVolume(machine.MachineTag(), volume.VolumeTag(), false)
		c.Assert(err, tc.ErrorIsNil)
...
func (s *VolumeStateSuite) TestDetachVolumeForce(c *tc.C) {
	volume, machine := s.setupModelScopedVolumeAttachment(c)
...
func (s *VolumeStateSuite) TestRemoveLastVolumeAttachment(c *tc.C) {
	volume, machine := s.setupModelScopedVolumeAttachment(c)
...
func (s *VolumeStateSuite) TestRemoveLastVolumeAttachmentConcurrently(c *tc.C) {
	volume, machine := s.setupModelScopedVolumeAttachment(c)
...
func (s *VolumeStateSuite) TestRemoveVolumeAttachmentNotFound(c *tc.C) {
	err := s.storageBackend.RemoveVolumeAttachment(names.NewMachineTag("42"), names.NewVolumeTag("42"), false)
	c.Assert(err, tc.Satisfies, errors.IsNotFound)
	c.Assert(err, tc.ErrorMatches, `removing attachment of volume 42 from machine 42: volume "42" on "machine 42" not found`)
}

func (s *VolumeStateSuite) TestRemoveVolumeAttachmentConcurrently(c *tc.C) {
	volume, machine := s.setupMachineScopedVolumeAttachment(c)
...
func (s *VolumeStateSuite) TestRemoveVolumeAttachmentAlive(c *tc.C) {
	volume, machine := s.setupMachineScopedVolumeAttachment(c)

	err := s.storageBackend.RemoveVolumeAttachment(machine.MachineTag(), volume.VolumeTag(), false)
	c.Assert(err, tc.ErrorMatches, "removing attachment of volume 0/0 from machine 0: volume attachment is not dying")
}

func (s *VolumeStateSuite) TestRemoveMachineRemovesVolumes(c *tc.C) {
	machine, err := s.State.AddOneMachine(state.MachineTemplate{
		Base: state.UbuntuBase("12.10"),
		Jobs: []state.MachineJob{state.JobHostUnits},
		Volumes: []state.HostVolumeParams{{
	...
	c.Assert(len(allVolumes), tc.GreaterThan, len(persistentVolumes))

	c.Assert(machine.Destroy(), tc.ErrorIsNil)

	// Cannot advance to Dead while there are detachable dynamic volumes.
	err = machine.EnsureDead()
	c.Assert(errors.Is(err, stateerrors.HasAttachmentsError), tc.IsTrue)
	c.Assert(err, tc.ErrorMatches, "machine 0 has attachments \\[volume-0\\]")
	...
	c.Assert(allVolumes, tc.HasLen, 1)
	c.Assert(allVolumes[0].Tag().String(), tc.Equals, "volume-0")
...
func (s *VolumeStateSuite) TestEnsureMachineDeadAddVolumeConcurrently(c *tc.C) {
	machine, err := s.State.AddOneMachine(state.MachineTemplate{
		Base: state.UbuntuBase("12.10"),
		Jobs: []state.MachineJob{state.JobHostUnits},
		Volumes: []state.HostVolumeParams{{
	...
	err = machine.EnsureDead()
	c.Assert(err, tc.ErrorMatches, `machine 0 has attachments \[volume-1\]`)
}

func (s *VolumeStateSuite) TestEnsureMachineDeadRemoveVolumeConcurrently(c *tc.C) {
	machine, err := s.State.AddOneMachine(state.MachineTemplate{
		Base: state.UbuntuBase("12.10"),
		Jobs: []state.MachineJob{state.JobHostUnits},
		Volumes: []state.HostVolumeParams{{
...
func (s *VolumeStateSuite) TestVolumeMachineScoped(c *tc.C) {
	machine, err := s.State.AddOneMachine(state.MachineTemplate{
		Base: state.UbuntuBase("12.10"),
		Jobs: []state.MachineJob{state.JobHostUnits},
		Volumes: []state.HostVolumeParams{{
...
func (s *VolumeStateSuite) TestVolumeBindingStorage(c *tc.C) {
...
func (s *VolumeStateSuite) setupStorageVolumeAttachment(c *tc.C) (state.Volume, *state.Machine, *state.Unit) {
	_, u, storageTag := s.setupSingleStorage(c, "block", "modelscoped")
	err := s.State.AssignUnit(u, state.AssignCleanEmpty)
	c.Assert(err, tc.ErrorIsNil)
	machine := unitMachine(c, s.State, u)
...
func (s *VolumeStateSuite) setupModelScopedVolumeAttachment(c *tc.C) (state.Volume, *state.Machine) {
	return s.setupVolumeAttachment(c, "modelscoped")
}

func (s *VolumeStateSuite) setupMachineScopedVolumeAttachment(c *tc.C) (state.Volume, *state.Machine) {
	return s.setupVolumeAttachment(c, "loop")
}

func (s *VolumeStateSuite) setupVolumeAttachment(c *tc.C, pool string) (state.Volume, *state.Machine) {
	machine, err := s.State.AddOneMachine(state.MachineTemplate{
		Base: state.UbuntuBase("12.10"),
		Jobs: []state.MachineJob{state.JobHostUnits},
		Volumes: []state.HostVolumeParams{{
...
func removeVolumeStorageInstance(c *tc.C, sb *state.StorageBackend, volumeTag names.VolumeTag) {
...
func removeStorageInstance(c *tc.C, sb *state.StorageBackend, storageTag names.StorageTag) {
