...
func SetModelTypeToCAAS(c *tc.C, st *State, m *Model) {
	ops := []txn.Op{{
		C:      modelsC,
		Id:     m.UUID(),
	...
	c.Assert(m.refresh(m.UUID()), tc.ErrorIsNil)
}

// AddTestingApplicationWithEmptyBindings mimics an application
// from an old version of Juju, with no bindings entry.
func AddTestingApplicationWithEmptyBindings(c *tc.C, st *State, name string, ch *Charm) *Application {
...
func RunTransaction(c *tc.C, st *State, ops []txn.Op) {
	c.Assert(st.db().RunTransaction(ops), tc.ErrorIsNil)
}
...
func MustOpenUnitPortRange(c *tc.C, st *State, machine *Machine, unitName, endpointName string, portRange network.PortRange) {
...
// for the specified unit and endpoint combination on the provided machine.
func MustOpenUnitPortRanges(c *tc.C, st *State, machine *Machine, unitName, endpointName string, portRanges []network.PortRange) {
	...
	c.Assert(st.ApplyOperation(machPortRanges.Changes()), tc.ErrorIsNil)
}

// MustCloseUnitPortRange ensures that the provided port range is closed
// for the specified unit and endpoint combination on the provided machine.
func MustCloseUnitPortRange(c *tc.C, st *State, machine *Machine, unitName, endpointName string, portRange network.PortRange) {
...
// for the specified unit and endpoint combination on the provided machine.
func MustCloseUnitPortRanges(c *tc.C, st *State, machine *Machine, unitName, endpointName string, portRanges []network.PortRange) {
	machPortRanges, err := machine.OpenedPortRanges()
	c.Assert(err, tc.ErrorIsNil)
...
func (st *State) IsSecretRevisionObsolete(c *tc.C, uri *secrets.URI, rev int) bool {
