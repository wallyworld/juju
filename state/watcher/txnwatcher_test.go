...
func TestTxnWatcherSuite(t *tctesting.T) {
	tc.Run(t, &TxnWatcherSuite{})
}

func (s *TxnWatcherSuite) SetUpSuite(c *tc.C) {
	s.MgoSuite.SetUpSuite(c)
	s.BaseSuite.SetUpSuite(c)
}

func (s *TxnWatcherSuite) TearDownSuite(c *tc.C) {
	s.BaseSuite.TearDownSuite(c)
	s.MgoSuite.TearDownSuite(c)
}

func (s *TxnWatcherSuite) SetUpTest(c *tc.C) {
	s.MgoSuite.SetUpTest(c)
	s.BaseSuite.SetUpTest(c)
	s.clock = testclock.NewDilatedWallClock(100 * time.Millisecond)
}

func (s *TxnWatcherSuite) TearDownTest(c *tc.C) {
	s.BaseSuite.TearDownTest(c)
	s.MgoSuite.TearDownTest(c)
}

func (s *TxnWatcherSuite) newWatcher(c *tc.C, expect int) (*watcher.TxnWatcher, *fakeHub) {
	return s.newWatcherWithError(c, expect, nil, watcher.TxnWatcherConfig{})
}

func (s *TxnWatcherSuite) newRunner(c *tc.C) {
...
func (s *TxnWatcherSuite) newWatcherWithError(c *tc.C, expect int, watcherError error, baseConfig watcher.TxnWatcherConfig) (*watcher.TxnWatcher, *fakeHub) {
	...
	s.AddCleanup(func(c *tc.C) {
		if watcherError == nil {
			c.Check(w.Stop(), tc.ErrorIsNil)
		} else {
			err := w.Stop()
			c.Check(errors.Is(err, watcherError), tc.IsTrue,
				tc.Commentf("%s should match error %s", err.Error(), watcherError.Error()))
		}
		session.Close()
	})
	err = w.Ready()
...
func (s *TxnWatcherSuite) revno(c *tc.C, coll string, id interface{}) (revno int64) {
	var doc struct {
...
func (s *TxnWatcherSuite) insert(c *tc.C, coll string, id interface{}) (revno int64) {
...
func (s *TxnWatcherSuite) insertAll(c *tc.C, coll string, ids ...interface{}) (revnos []int64) {
	var ops []txn.Op
	for _, id := range ids {
...
func (s *TxnWatcherSuite) update(c *tc.C, coll string, id interface{}) (revno int64) {
...
func (s *TxnWatcherSuite) updateTwice(c *tc.C, coll string, id interface{}) (revno int64) {
	ops := []txn.Op{
...
func (s *TxnWatcherSuite) remove(c *tc.C, coll string, id interface{}) (revno int64) {
...
func (s *TxnWatcherSuite) TestErrAndDead(c *tc.C) {
	w, _ := s.newWatcher(c, 0)
	c.Assert(w.Err(), tc.Equals, tomb.ErrStillAlive)
	select {
	case <-w.Dead():
		c.Fatalf("Dead channel fired unexpectedly")
	default:
	}
	c.Assert(w.Stop(), tc.ErrorIsNil)
	c.Assert(w.Err(), tc.ErrorIsNil)
	select {
	case <-w.Dead():
	default:
...
func (s *TxnWatcherSuite) TestInsert(c *tc.C) {
	s.newRunner(c)
	...
	c.Assert(hub.values, tc.DeepEquals, []watcher.Change{
		{"test", "a", revno},
	})
}

func (s *TxnWatcherSuite) TestUpdate(c *tc.C) {
	s.newRunner(c)
	s.insert(c, "test", "a")
...
func (s *TxnWatcherSuite) TestRemove(c *tc.C) {
	s.newRunner(c)
	s.insert(c, "test", "a")
...
func (s *TxnWatcherSuite) TestWatchOrder(c *tc.C) {
	s.newRunner(c)
...
func (s *TxnWatcherSuite) TestTransactionWithMultiple(c *tc.C) {
	s.newRunner(c)
	_, hub := s.newWatcher(c, 3)
...
func (s *TxnWatcherSuite) TestScale(c *tc.C) {
	const N = 500
	const T = 10
	...
	c.Assert(count, tc.Equals, N)

	hub.waitForExpected()

	for i := 0; i < N; i++ {
		c.Assert(hub.values[i].Id, tc.Equals, i)
	}
}

func (s *TxnWatcherSuite) TestInsertThenRemove(c *tc.C) {
	s.newRunner(c)
...
func (s *TxnWatcherSuite) TestMultiUpdateSameDocSameTxn(c *tc.C) {
	s.newRunner(c)
	_, hub := s.newWatcher(c, 2)
...
func (s *TxnWatcherSuite) TestShouldRetryGetMore(c *tc.C) {
	s.newRunner(c)
	...
	s.insert(c, "test", "a")
	hub.waitForExpected()
	c.Assert(atomic.LoadInt32(&numResumeOrStart), tc.Equals, int32(2))
}

func (s *TxnWatcherSuite) TestShouldResume(c *tc.C) {
	s.newRunner(c)
...
func (s *TxnWatcherSuite) TestNotResumable(c *tc.C) {
	s.newRunner(c)
	numResumeOrStart := int32(0)
	run := func(db *mgo.Database, cmd, resp any) error {
		b, ok := cmd.(bson.D)
	...
	s.insert(c, "test", "a")
	hub.waitForError()
	c.Assert(atomic.LoadInt32(&numResumeOrStart), tc.Equals, int32(1))
}

func (s *TxnWatcherSuite) TestFilterCollection(c *tc.C) {
	s.newRunner(c)
...
func newFakeHub(c *tc.C, expected int) *fakeHub {
	return &fakeHub{
		c:      c,
		expect: expected,
