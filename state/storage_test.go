...
func (s *StorageStateSuiteBase) SetUpTest(c *tc.C) {
...
func (s *StorageStateSuiteBase) AddTestingCharm(c *tc.C, name string) *state.Charm {
	return state.AddTestingCharmForSeries(c, s.st, s.series, name)
}

func (s *StorageStateSuiteBase) AddTestingApplication(c *tc.C, name string, ch *state.Charm) *state.Application {
	return state.AddTestingApplicationForBase(c, s.st, s.base, name, ch)
}

func (s *StorageStateSuiteBase) AddTestingApplicationWithStorage(c *tc.C, name string, ch *state.Charm, storage map[string]state.StorageConstraints) *state.Application {
	return state.AddTestingApplicationWithStorage(c, s.st, name, ch, storage)
}

func (s *StorageStateSuiteBase) AddMetaCharm(c *tc.C, name, metaYaml string, revision int) *state.Charm {
	return state.AddCustomCharm(c, s.st, name, "metadata.yaml", metaYaml, s.series, revision)
}

func (s *StorageStateSuiteBase) setupSingleStorage(c *tc.C, kind, pool string) (*state.Application, *state.Unit, names.StorageTag) {
...
func (s *StorageStateSuiteBase) provisionStorageVolume(c *tc.C, u *state.Unit, storageTag names.StorageTag) {
...
func (s *StorageStateSuiteBase) setupSingleStorageDetachable(c *tc.C, kind, pool string) (*state.Application, *state.Unit, names.StorageTag) {
...
func (s *StorageStateSuiteBase) createStorageCharmWithSeries(c *tc.C, charmName string, storageMeta charm.Storage, series string) *state.Charm {
	return s.createStorageCharmRevMeta(c, charmName, storageMeta, 1, fmt.Sprintf("series: [%s]", series))
}

func (s *StorageStateSuiteBase) createStorageCharm(c *tc.C, charmName string, storageMeta charm.Storage) *state.Charm {
	return s.createStorageCharmRev(c, charmName, storageMeta, 1)
}

func (s *StorageStateSuiteBase) createStorageCharmRev(c *tc.C, charmName string, storageMeta charm.Storage, rev int) *state.Charm {
	return s.createStorageCharmRevMeta(c, charmName, storageMeta, rev, "")
}

func (s *StorageStateSuiteBase) createStorageCharmRevMeta(c *tc.C, charmName string, storageMeta charm.Storage, rev int, additionalMeta string) *state.Charm {
	meta := fmt.Sprintf(`
name: %s
...
func (s *StorageStateSuiteBase) setupMixedScopeStorageApplication(
	c *tc.C, kind string, pools ...string,
) *state.Application {
	pool0 := "modelscoped"
	pool1 := "machinescoped"
...
func (s *StorageStateSuiteBase) storageInstanceExists(c *tc.C, tag names.StorageTag) bool {
...
func (s *StorageStateSuiteBase) assertFilesystemUnprovisioned(c *tc.C, tag names.FilesystemTag) {
...
func (s *StorageStateSuiteBase) assertFilesystemInfo(c *tc.C, tag names.FilesystemTag, expect state.FilesystemInfo) {
	filesystem := s.filesystem(c, tag)
	info, err := filesystem.Info()
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(info, tc.DeepEquals, expect)
	_, ok := filesystem.Params()
	c.Assert(ok, tc.IsFalse)
}

func (s *StorageStateSuiteBase) assertFilesystemAttachmentUnprovisioned(c *tc.C, host names.Tag, f names.FilesystemTag) {
...
func (s *StorageStateSuiteBase) assertFilesystemAttachmentInfo(c *tc.C, host names.Tag, f names.FilesystemTag, expect state.FilesystemAttachmentInfo) {
	filesystemAttachment := s.filesystemAttachment(c, host, f)
...
func (s *StorageStateSuiteBase) assertVolumeUnprovisioned(c *tc.C, tag names.VolumeTag) {
...
func (s *StorageStateSuiteBase) assertVolumeInfo(c *tc.C, tag names.VolumeTag, expect state.VolumeInfo) {
	volume := s.volume(c, tag)
...
func (s *StorageStateSuiteBase) filesystem(c *tc.C, tag names.FilesystemTag) state.Filesystem {
	filesystem, err := s.storageBackend.Filesystem(tag)
	c.Assert(err, tc.ErrorIsNil)
	return filesystem
}

func (s *StorageStateSuiteBase) filesystemVolume(c *tc.C, tag names.FilesystemTag) state.Volume {
	filesystem := s.filesystem(c, tag)
...
func (s *StorageStateSuiteBase) filesystemAttachment(c *tc.C, host names.Tag, f names.FilesystemTag) state.FilesystemAttachment {
	attachment, err := s.storageBackend.FilesystemAttachment(host, f)
	c.Assert(err, tc.ErrorIsNil)
	return attachment
}

func (s *StorageStateSuiteBase) volume(c *tc.C, tag names.VolumeTag) state.Volume {
	volume, err := s.storageBackend.Volume(tag)
	c.Assert(err, tc.ErrorIsNil)
	return volume
}

func (s *StorageStateSuiteBase) volumeFilesystem(c *tc.C, tag names.VolumeTag) state.Filesystem {
	filesystem, err := s.storageBackend.VolumeFilesystem(tag)
	c.Assert(err, tc.ErrorIsNil)
	return filesystem
}

func (s *StorageStateSuiteBase) volumeAttachment(c *tc.C, host names.Tag, v names.VolumeTag) state.VolumeAttachment {
	attachment, err := s.storageBackend.VolumeAttachment(host, v)
	c.Assert(err, tc.ErrorIsNil)
	return attachment
}

func (s *StorageStateSuiteBase) volumeAttachmentPlan(c *tc.C, host names.Tag, v names.VolumeTag) state.VolumeAttachmentPlan {
	attachmentPlan, err := s.storageBackend.VolumeAttachmentPlan(host, v)
	c.Assert(err, tc.ErrorIsNil)
	return attachmentPlan
}

func (s *StorageStateSuiteBase) storageInstanceVolume(c *tc.C, tag names.StorageTag) state.Volume {
	volume, err := s.storageBackend.StorageInstanceVolume(tag)
	c.Assert(err, tc.ErrorIsNil)
	return volume
}

func (s *StorageStateSuiteBase) storageInstanceFilesystem(c *tc.C, tag names.StorageTag) state.Filesystem {
	filesystem, err := s.storageBackend.StorageInstanceFilesystem(tag)
	c.Assert(err, tc.ErrorIsNil)
	return filesystem
}

func (s *StorageStateSuiteBase) obliterateUnit(c *tc.C, tag names.UnitTag) {
...
func (s *StorageStateSuiteBase) obliterateUnitStorage(c *tc.C, tag names.UnitTag) {
...
func (s *StorageStateSuiteBase) obliterateVolume(c *tc.C, tag names.VolumeTag) {
...
func (s *StorageStateSuiteBase) obliterateVolumeAttachment(c *tc.C, m names.Tag, v names.VolumeTag) {
...
func (s *StorageStateSuiteBase) obliterateFilesystem(c *tc.C, tag names.FilesystemTag) {
...
func (s *StorageStateSuiteBase) obliterateFilesystemAttachment(c *tc.C, host names.Tag, f names.FilesystemTag) {
...
func assertMachineStorageRefs(c *tc.C, sb *state.StorageBackend, m names.MachineTag) {
	...
	c.Assert(have, tc.DeepEquals, expect)
}
...
func TestStorageStateSuite(t *tctesting.T) {
	testing.MgoTestPackage(t, &StorageStateSuite{})
}

func (s *StorageStateSuite) TestBlockStorageNotSupportedOnCAAS(c *tc.C) {
	...
	c.Assert(err, tc.ErrorMatches, `cannot add application "storage-block": block storage on a container model not supported`)
}

func (s *StorageStateSuite) TestAddApplicationStorageConstraintsDefault(c *tc.C) {
	...
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(constraints, tc.DeepEquals, map[string]state.StorageConstraints{
		"data": {
			Pool:  "loop",
			Count: 1,
			Size:  1024,
...
func (s *StorageStateSuite) TestAddApplicationStorageConstraintsValidation(c *tc.C) {
...
func (s *StorageStateSuite) assertAddApplicationStorageConstraintsDefaults(c *tc.C, pool string, cons, expect map[string]state.StorageConstraints) {
	if pool != "" {
	...
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(savedCons, tc.DeepEquals, expect)
	// TODO(wallyworld) - test pool name stored in data model
}

func (s *StorageStateSuite) TestAddApplicationStorageConstraintsNoConstraintsUsed(c *tc.C) {
	storageCons := map[string]state.StorageConstraints{
...
func (s *StorageStateSuite) TestAddApplicationStorageConstraintsJustCount(c *tc.C) {
	storageCons := map[string]state.StorageConstraints{
...
func (s *StorageStateSuite) TestAddApplicationStorageConstraintsDefaultPool(c *tc.C) {
	storageCons := map[string]state.StorageConstraints{
...
func (s *StorageStateSuite) TestAddApplicationStorageConstraintsNoUserDefaultPool(c *tc.C) {
	storageCons := map[string]state.StorageConstraints{
		"data": makeStorageCons("", 2048, 1),
	}
	expectedCons := map[string]state.StorageConstraints{
...
func (s *StorageStateSuite) TestAddApplicationStorageConstraintsDefaultSizeFallback(c *tc.C) {
	storageCons := map[string]state.StorageConstraints{
...
func (s *StorageStateSuite) TestAddApplicationStorageConstraintsDefaultSizeFromCharm(c *tc.C) {
	storageCons := map[string]state.StorageConstraints{
	...
	savedCons, err := app.StorageConstraints()
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(savedCons, tc.DeepEquals, expectedCons)
}

func (s *StorageStateSuite) TestProviderFallbackToType(c *tc.C) {
	ch := s.AddTestingCharm(c, "storage-block")
	addApplication := func(storage map[string]state.StorageConstraints) (*state.Application, error) {
		return s.st.AddApplication(state.AddApplicationArgs{
			Name: "storage-block", Charm: ch,
...
func (s *StorageStateSuite) TestAddUnit(c *tc.C) {
	s.assertStorageUnitsAdded(c)
}

func (s *StorageStateSuite) assertStorageUnitsAdded(c *tc.C) {
	err := s.Model.UpdateModelConfig(map[string]interface{}{
	...
	for i := 0; i < 2; i++ {
		...
		for _, att := range storageAttachments {
			c.Assert(att.Unit(), tc.Equals, u.UnitTag())
			storageInstance, err := s.storageBackend.StorageInstance(att.StorageInstance())
			c.Assert(err, tc.ErrorIsNil)
			count[storageInstance.StorageName()]++
			c.Assert(storageInstance.Kind(), tc.Equals, state.StorageKindBlock)
		}
		c.Assert(count, tc.DeepEquals, map[string]int{
			"multi1to10": 1,
			"multi2up":   2,
		})
		// TODO(wallyworld) - test pool name stored in data model
...
func (s *StorageStateSuite) TestAllStorageInstances(c *tc.C) {
	s.assertStorageUnitsAdded(c)

	all, err := s.storageBackend.AllStorageInstances()
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(all, tc.HasLen, 6)

	nameSet := set.NewStrings("multi1to10", "multi2up")
	ownerSet := set.NewStrings("unit-storage-block2-0", "unit-storage-block2-1")

	for _, one := range all {
		c.Assert(one.Kind(), tc.DeepEquals, state.StorageKindBlock)
		c.Assert(nameSet.Contains(one.StorageName()), tc.IsTrue)
		owner, ok := one.Owner()
		c.Assert(ok, tc.IsTrue)
		c.Assert(ownerSet.Contains(owner.String()), tc.IsTrue)
	}
}

func (s *StorageStateSuite) TestRemoveStoragePool(c *tc.C) {
	poolName := "loop-pool"
	...
	c.Assert(err, tc.ErrorIsNil)
	_, err = s.pm.Get(poolName)
	c.Assert(err, tc.ErrorMatches, fmt.Sprintf("pool %q not found", poolName))
}

func (s *StorageStateSuite) TestRemoveStoragePoolInUse(c *tc.C) {
	poolName := "loop-pool"
	s.assertStorageUnitsAdded(c)
	_, err := s.pm.Get(poolName)
	c.Assert(err, tc.ErrorIsNil)
	err = s.storageBackend.RemoveStoragePool(poolName)
	c.Assert(err, tc.ErrorMatches, fmt.Sprintf("storage pool %q in use", poolName))
	pool, err := s.pm.Get(poolName)
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(pool.Name(), tc.Equals, poolName)

	_, err = s.pm.Get("nope-pool")
	c.Assert(err, tc.ErrorMatches, `pool "nope-pool" not found`)
	err = s.storageBackend.RemoveStoragePool("nope-pool")
	c.Assert(err, tc.ErrorMatches, `storage pool "nope-pool" not found`)
}

func (s *StorageStateSuite) TestStorageAttachments(c *tc.C) {
	s.assertStorageUnitsAdded(c)
	...
	assertAttachments := func(tag names.StorageTag, expect ...names.UnitTag) {
		...
		c.Assert(units, tc.SameContents, expect)
	}
...
func (s *StorageStateSuite) TestAllStorageInstancesEmpty(c *tc.C) {
	all, err := s.storageBackend.AllStorageInstances()
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(all, tc.HasLen, 0)
}

func (s *StorageStateSuite) TestUnitEnsureDead(c *tc.C) {
	if s.series == "kubernetes" {
	...
	assertUnitEnsureDeadError := func() {
		err = u.EnsureDead()
		c.Assert(err, tc.ErrorMatches, "unit has storage attachments")
	}
...
func (s *StorageStateSuite) TestUnitStorageProvisionerError(c *tc.C) {
	if s.series == "kubernetes" {
		c.Skip("volumes on containers not supported")
	}
	_, u, storageTag := s.setupSingleStorage(c, "block", "loop-pool")
...
func (s *StorageStateSuite) TestRemoveStorageAttachmentsRemovesDyingInstance(c *tc.C) {
	if s.series == "kubernetes" {
		c.Skip("volumes on containers not supported")
	}
	_, u, storageTag := s.setupSingleStorage(c, "block", "loop-pool")
...
func (s *StorageStateSuite) TestRemoveStorageAttachmentsDisownsUnitOwnedInstance(c *tc.C) {
	if s.series == "kubernetes" {
		c.Skip("volumes on containers not supported")
	}
	...
	volume := s.storageInstanceVolume(c, storageTag)
	c.Assert(volume.Life(), tc.Equals, state.Alive)
	volumeAttachment := s.volumeAttachment(c, machineTag, volume.VolumeTag())
	c.Assert(volumeAttachment.Life(), tc.Equals, state.Dying)
}

func (s *StorageStateSuite) TestAttachStorageTakesOwnership(c *tc.C) {
	...
	c.Assert(hasOwner, tc.IsTrue)
	c.Assert(owner, tc.Equals, u2.Tag())
}

func (s *StorageStateSuite) TestAttachStorageIdempotent(c *tc.C) {
	app, u, storageTag := s.setupSingleStorageDetachable(c, "block", "modelscoped")
	u2, err := app.AddUnit(state.AddUnitParams{})
	c.Assert(err, tc.ErrorIsNil)
...
func (s *StorageStateSuite) TestAttachStorageAssignedMachine(c *tc.C) {
	app, u, storageTag := s.setupSingleStorageDetachable(c, "block", "modelscoped")
	u2, err := app.AddUnit(state.AddUnitParams{})
	c.Assert(err, tc.ErrorIsNil)
...
func (s *StorageStateSuite) TestAttachStorageAssignedMachineExistingVolume(c *tc.C) {
...
func (s *StorageStateSuite) TestAttachStorageAssignedMachineExistingVolumeAttached(c *tc.C) {
	app, u, storageTag := s.setupSingleStorageDetachable(c, "block", "modelscoped")
	u2, err := app.AddUnit(state.AddUnitParams{})
	c.Assert(err, tc.ErrorIsNil)
...
func (s *StorageStateSuite) TestAddApplicationAttachStorage(c *tc.C) {
	app, u, storageTag := s.setupSingleStorageDetachable(c, "block", "modelscoped")
	...
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(app2Units, tc.HasLen, 1)
	...
	owner, hasOwner := storageInstance.Owner()
	c.Assert(hasOwner, tc.IsTrue)
	c.Assert(owner, tc.Equals, app2Units[0].UnitTag())
...
func (s *StorageStateSuite) TestAddApplicationAttachStorageMultipleUnits(c *tc.C) {
	...
	c.Assert(err, tc.ErrorMatches, `cannot add application "secondwind": AttachStorage is non-empty but NumUnits is 2, must be 1`)
}

func (s *StorageStateSuite) TestAddApplicationAttachStorageTooMany(c *tc.C) {
...
func (s *StorageStateSuite) TestAddUnitAttachStorage(c *tc.C) {
	app, u, storageTag := s.setupSingleStorageDetachable(c, "block", "modelscoped")
	...
	owner, hasOwner := storageInstance.Owner()
	c.Assert(hasOwner, tc.IsTrue)
	c.Assert(owner, tc.Equals, u2.UnitTag())
}

func (s *StorageStateSuite) TestConcurrentDestroyStorageInstanceRemoveStorageAttachmentsRemovesInstance(c *tc.C) {
	if s.series == "kubernetes" {
		c.Skip("volumes on containers not supported")
	}
	_, u, storageTag := s.setupSingleStorage(c, "block", "loop-pool")
...
func (s *StorageStateSuite) TestConcurrentRemoveStorageAttachment(c *tc.C) {
	if s.series == "kubernetes" {
		c.Skip("volumes on containers not supported")
	}
	_, u, storageTag := s.setupSingleStorage(c, "block", "loop-pool")
...
func (s *StorageStateSuite) TestRemoveAliveStorageAttachmentError(c *tc.C) {
	if s.series == "kubernetes" {
		c.Skip("volumes on containers not supported")
	}
	_, u, storageTag := s.setupSingleStorage(c, "block", "loop-pool")
	...
	err := s.storageBackend.RemoveStorageAttachment(storageTag, u.UnitTag(), false)
	c.Assert(err, tc.ErrorMatches, "cannot remove storage attachment data/0:storage-block/0: storage attachment is not dying")

	attachments, err := s.storageBackend.UnitStorageAttachments(u.UnitTag())
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(attachments, tc.HasLen, 1)
	c.Assert(attachments[0].StorageInstance(), tc.Equals, storageTag)
}

func (s *StorageStateSuite) TestConcurrentDestroyInstanceRemoveStorageAttachmentsRemovesInstance(c *tc.C) {
	if s.series == "kubernetes" {
		c.Skip("volumes on containers not supported")
	}
	_, u, storageTag := s.setupSingleStorage(c, "block", "loop-pool")
...
func (s *StorageStateSuite) TestConcurrentDestroyStorageInstance(c *tc.C) {
	if s.series == "kubernetes" {
		c.Skip("volumes on containers not supported")
	}
	_, u, storageTag := s.setupSingleStorage(c, "block", "loop-pool")
...
func (s *StorageStateSuite) TestDestroyStorageInstanceNotFound(c *tc.C) {
	err := s.storageBackend.DestroyStorageInstance(names.NewStorageTag("foo/0"), true, false, dontWait)
	c.Assert(err, tc.ErrorMatches, `cannot destroy storage "foo/0": storage instance "foo/0" not found`)
	c.Assert(err, tc.Satisfies, errors.IsNotFound)
}

func (s *StorageStateSuite) TestDestroyStorageInstanceAttachedError(c *tc.C) {
	if s.series == "kubernetes" {
		c.Skip("volumes on containers not supported")
	}
	...
	c.Assert(err, tc.ErrorMatches, `cannot destroy storage "data/0": storage is attached`)
	c.Assert(errors.Is(err, stateerrors.StorageAttachedError), tc.IsTrue)
}

func (s *StorageStateSuite) TestWatchStorageAttachments(c *tc.C) {
	ch := s.AddTestingCharm(c, "storage-block2")
	testStorage := map[string]state.StorageConstraints{
...
func (s *StorageStateSuite) TestWatchStorageAttachment(c *tc.C) {
	if s.series == "kubernetes" {
		c.Skip("volumes on containers not supported")
	}
	_, u, storageTag := s.setupSingleStorage(c, "block", "loop-pool")
...
func (s *StorageStateSuite) TestDestroyUnitStorageAttachments(c *tc.C) {
	...
	defer state.SetBeforeHooks(c, s.st, func() {
		...
		for _, a := range attachments {
			c.Assert(a.Life(), tc.Equals, state.Dying)
...
func (s *StorageStateSuite) TestStorageLocationConflictIdentical(c *tc.C) {
...
func (s *StorageStateSuite) TestStorageLocationConflictIdenticalAfterCleaning(c *tc.C) {
	s.testStorageLocationConflict(
		c, "/srv", "/xyz/.././srv",
		`cannot assign unit "storage-filesystem2/0" to machine 0: `+
			`validating filesystem mount points: `+
...
func (s *StorageStateSuite) TestStorageLocationConflictSecondInsideFirst(c *tc.C) {
	s.testStorageLocationConflict(
		c, "/srv", "/srv/within",
		`cannot assign unit "storage-filesystem2/0" to machine 0: `+
			`validating filesystem mount points: `+
...
func (s *StorageStateSuite) TestStorageLocationConflictFirstInsideSecond(c *tc.C) {
	s.testStorageLocationConflict(
		c, "/srv/within", "/srv",
		`cannot assign unit "storage-filesystem2/0" to machine 0: `+
			`validating filesystem mount points: `+
...
func (s *StorageStateSuite) TestStorageLocationConflictPrefix(c *tc.C) {
	s.testStorageLocationConflict(c, "/srv", "/srvtd", "")
}

func (s *StorageStateSuite) TestStorageLocationConflictSameParent(c *tc.C) {
	s.testStorageLocationConflict(c, "/srv/1", "/srv/2", "")
}

func (s *StorageStateSuite) TestStorageLocationConflictAutoGenerated(c *tc.C) {
	s.testStorageLocationConflict(c, "", "", "")
}

func (s *StorageStateSuite) testStorageLocationConflict(c *tc.C, first, second, expectErr string) {
...
func (s *StorageStateSuite) TestNewModelDefaultPools(c *tc.C) {
	...
	c.Assert(listed, tc.DeepEquals, []*storage.Config{blackPool, radiancePool})
}
...
func TestStorageStateSuiteCaas(t *tctesting.T) {
	testing.MgoTestPackage(t, &StorageStateSuiteCaas{})
}

func (s *StorageStateSuiteCaas) SetUpTest(c *tc.C) {
	s.series = "kubernetes"
	s.StorageStateSuiteBase.SetUpTest(c)
}

func (s *StorageStateSuiteCaas) TestRemoveStoragePool(c *tc.C) {
...
func (s *StorageStateSuiteCaas) TestRemoveStoragePoolInUse(c *tc.C) {
	...
	c.Assert(err, tc.ErrorMatches, fmt.Sprintf("storage pool %q in use", appPoolName))
	pool, err = s.pm.Get(poolName)
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(pool.Name(), tc.Equals, poolName)
}

func (s *StorageStateSuiteCaas) TestDeployWrongStorageType(c *tc.C) {
	ch := s.AddTestingCharm(c, "storage-filesystem")
	...
	c.Assert(err, tc.ErrorMatches, `cannot add application "foo": invalid storage config: storage provider type "loop" not valid`)
}
...
func TestStorageSubordinateStateSuite(t *tctesting.T) {
	testing.MgoTestPackage(t, &StorageSubordinateStateSuite{})
}

func (s *StorageSubordinateStateSuite) SetUpTest(c *tc.C) {
	s.StorageStateSuiteBase.SetUpTest(c)
...
func (s *StorageSubordinateStateSuite) TestSubordinateStoragePrincipalUnassigned(c *tc.C) {
	storageTag := names.NewStorageTag("data/0")
	exists := s.storageInstanceExists(c, storageTag)
	c.Assert(exists, tc.IsFalse)
...
func (s *StorageSubordinateStateSuite) TestSubordinateStoragePrincipalAssigned(c *tc.C) {
	err := s.st.AssignUnit(s.mysqlUnit, state.AssignCleanEmpty)
	c.Assert(err, tc.ErrorIsNil)
...
func (s *StorageSubordinateStateSuite) TestSubordinateStoragePrincipalAssignRace(c *tc.C) {
