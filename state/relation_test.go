...
func TestRelationSuite(t *tctesting.T) {
	testing.MgoTestPackage(t, &RelationSuite{})
}

func (s *RelationSuite) TestAddRelationErrors(c *tc.C) {
	...
	c.Assert(err, tc.ErrorMatches, `cannot add relation "wordpress:db yoursql:server": application "yoursql" does not exist`)
	...
	c.Assert(err, tc.ErrorMatches, `cannot add relation "wordpress:db mysql:server": endpoints do not relate`)
	assertNoRelations(c, wordpress)
	assertNoRelations(c, mysql)

	// Check a variety of surprising endpoint combinations.
	_, err = s.State.AddRelation(wordpressEP)
	c.Assert(err, tc.ErrorMatches, `cannot add relation "wordpress:db": relation must have two endpoints`)
	assertNoRelations(c, wordpress)

	_, err = s.State.AddRelation(riakEP, wordpressEP)
	c.Assert(err, tc.ErrorMatches, `cannot add relation "wordpress:db riak:ring": endpoints do not relate`)
	assertOneRelation(c, riak, 0, riakEP)
	assertNoRelations(c, wordpress)

	_, err = s.State.AddRelation(riakEP, riakEP)
	c.Assert(err, tc.ErrorMatches, `cannot add relation "riak:ring riak:ring": endpoints do not relate`)
	assertOneRelation(c, riak, 0, riakEP)

	_, err = s.State.AddRelation()
	c.Assert(err, tc.ErrorMatches, `cannot add relation "": relation must have two endpoints`)
	_, err = s.State.AddRelation(mysqlEP, wordpressEP, riakEP)
	c.Assert(err, tc.ErrorMatches, `cannot add relation "wordpress:db mysql:server riak:ring": relation must have two endpoints`)
	assertOneRelation(c, riak, 0, riakEP)
	assertNoRelations(c, wordpress)
	assertNoRelations(c, mysql)
	...
	c.Assert(err, tc.ErrorMatches, `cannot add relation "wordpress:db mysql:server": application "wordpress" is not alive`)
	assertNoRelations(c, wordpress)
	assertNoRelations(c, mysql)
}

func (s *StateSuite) TestAddRelationWithMaxLimit(c *tc.C) {
	...
	c.Assert(err, tc.ErrorIsNil)
	_, err = s.State.AddRelation(eps...)
	c.Assert(err, tc.Satisfies, errors.IsQuotaLimitExceeded, tc.Commentf("expected second AddRelation attempt to fail due to the limit:1 entry in the wordpress charm's metadata.yaml"))
}

func (s *RelationSuite) TestRetrieveSuccess(c *tc.C) {
	wordpress := s.AddTestingApplication(c, "wordpress", s.AddTestingCharm(c, "wordpress"))
	wordpressEP, err := wordpress.Endpoint("db")
	c.Assert(err, tc.ErrorIsNil)
	mysql := s.AddTestingApplication(c, "mysql", s.AddTestingCharm(c, "mysql"))
	...
	check := func() {
		c.Assert(err, tc.ErrorIsNil)
		c.Assert(rel.Id(), tc.Equals, expect.Id())
		c.Assert(rel.String(), tc.Equals, expect.String())
		c.Assert(rel.UnitCount(), tc.Equals, 1)
	}
	check()
...
func (s *RelationSuite) TestRetrieveNotFound(c *tc.C) {
	...
	c.Assert(err, tc.ErrorMatches, `relation "subway:db mongo:server" not found`)
	c.Assert(err, tc.Satisfies, errors.IsNotFound)

	_, err = s.State.Relation(999)
	c.Assert(err, tc.ErrorMatches, `relation 999 not found`)
	c.Assert(err, tc.Satisfies, errors.IsNotFound)
}

func (s *RelationSuite) TestAddRelation(c *tc.C) {
	// Add a relation.
	wordpress := s.AddTestingApplication(c, "wordpress", s.AddTestingCharm(c, "wordpress"))
	wordpressEP, err := wordpress.Endpoint("db")
	c.Assert(err, tc.ErrorIsNil)
...
func (s *RelationSuite) TestAddRelationSeriesNeedNotMatch(c *tc.C) {
	wordpress := s.AddTestingApplication(c, "wordpress", s.AddTestingCharm(c, "wordpress"))
	wordpressEP, err := wordpress.Endpoint("db")
	c.Assert(err, tc.ErrorIsNil)
...
func (s *RelationSuite) TestAddContainerRelation(c *tc.C) {
	// Add a relation.
	wordpress := s.AddTestingApplication(c, "wordpress", s.AddTestingCharm(c, "wordpress"))
	...
	c.Assert(err, tc.ErrorMatches, `cannot add relation "logging:info wordpress:juju-info": relation logging:info wordpress:juju-info`)
	_, err = s.State.AddRelation(wordpressEP, loggingEP)
	c.Assert(err, tc.ErrorMatches, `cannot add relation "logging:info wordpress:juju-info": relation logging:info wordpress:juju-info`)
	assertOneRelation(c, logging, 0, loggingEP, wordpressEP)
	assertOneRelation(c, wordpress, 0, wordpressEP, loggingEP)
...
func (s *RelationSuite) TestAddContainerRelationSeriesMustMatch(c *tc.C) {
	wordpress := s.AddTestingApplication(c, "wordpress", s.AddTestingCharm(c, "wordpress"))
	wordpressEP, err := wordpress.Endpoint("juju-info")
	c.Assert(err, tc.ErrorIsNil)
	...
	c.Assert(err, tc.ErrorIsNil)
	_, err = s.State.AddRelation(wordpressEP, loggingEP)
	c.Assert(err, tc.ErrorMatches, `cannot add relation "logging:info wordpress:juju-info": principal and subordinate applications' bases must match`)
}

func (s *RelationSuite) TestAddContainerRelationMultiSeriesMatch(c *tc.C) {
...
func (s *RelationSuite) TestAddContainerRelationMultiSeriesNoMatch(c *tc.C) {
...
`[1:]
	...
	_, err = s.State.AddRelation(principalEP, subordEP)
	principalEP.Scope = charm.ScopeContainer
	c.Assert(err, tc.ErrorMatches, `cannot add relation "multi-series-subordinate:multi-directory multi-series:multi-directory": principal and subordinate applications' bases must match`)
}

func (s *RelationSuite) TestAddContainerRelationWithNoSubordinate(c *tc.C) {
	wordpress := s.AddTestingApplication(c, "wordpress", s.AddTestingCharm(c, "wordpress"))
...
func (s *RelationSuite) TestAddContainerRelationWithTwoSubordinates(c *tc.C) {
...
func (s *RelationSuite) TestDestroyRelation(c *tc.C) {
	wordpress := s.AddTestingApplication(c, "wordpress", s.AddTestingCharm(c, "wordpress"))
	mysql := s.AddTestingApplication(c, "mysql", s.AddTestingCharm(c, "mysql"))
	eps, err := s.State.InferEndpoints("wordpress", "mysql")
	c.Assert(err, tc.ErrorIsNil)
...
func (s *RelationSuite) TestDestroyPeerRelation(c *tc.C) {
	...
	err = rel.Destroy()
	c.Assert(err, tc.ErrorMatches, `cannot destroy relation "riak:ring": is a peer relation`)
	assertOneRelation(c, riak, 0, riakEP)
...
func (s *RelationSuite) TestDestroyRelationIncorrectUnitCount(c *tc.C) {
	...
	c.Assert(err, tc.ErrorMatches, ".*unit count mismatch on relation wordpress:db mysql:server: expected 4 units in scope but got 0")
}

func (s *RelationSuite) assertInScope(c *tc.C, relUnit *state.RelationUnit, inScope bool) {
	ok, err := relUnit.InScope()
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(ok, tc.Equals, inScope)
}

func (s *RelationSuite) assertDestroyCrossModelRelation(c *tc.C, appStatus *status.Status) {
...
func (s *RelationSuite) TestDestroyCrossModelRelationNoAppStatus(c *tc.C) {
	s.assertDestroyCrossModelRelation(c, nil)
}

func (s *RelationSuite) TestDestroyCrossModelRelationAppNotTerminated(c *tc.C) {
	st := status.Active
	s.assertDestroyCrossModelRelation(c, &st)
}

func (s *RelationSuite) TestDestroyCrossModelRelationAppTerminated(c *tc.C) {
	st := status.Terminated
	s.assertDestroyCrossModelRelation(c, &st)
}

func (s *RelationSuite) TestForceDestroyCrossModelRelationOfferSide(c *tc.C) {
	rwordpress, err := s.State.AddRemoteApplication(state.AddRemoteApplicationParams{
		Name:                   "remote-wordpress",
...
func (s *RelationSuite) TestIsCrossModelYup(c *tc.C) {
	rwordpress, err := s.State.AddRemoteApplication(state.AddRemoteApplicationParams{
		Name:            "remote-wordpress",
		SourceModel:     names.NewModelTag("source-model"),
		IsConsumerProxy: true,
	...
	c.Assert(app.Name(), tc.Equals, "remote-wordpress")
}

func (s *RelationSuite) TestAddCrossModelNotAllowed(c *tc.C) {
	rwordpress, err := s.State.AddRemoteApplication(state.AddRemoteApplicationParams{
		Name:        "remote-wordpress",
		SourceModel: names.NewModelTag("source-model"),
		OfferUUID:   "offer-uuid",
	...
	c.Assert(err, tc.ErrorIsNil)
	_, err = s.State.AddRelation(wordpressEP, mysqlEP)
	c.Assert(err, tc.ErrorMatches, `cannot add relation "remote-wordpress:db mysql:server": remote offer remote-wordpress is terminated`)

}

func (s *RelationSuite) TestIsCrossModelNope(c *tc.C) {
	wordpress := s.AddTestingApplication(c, "wordpress", s.AddTestingCharm(c, "wordpress"))
	wordpressEP, err := wordpress.Endpoint("db")
	c.Assert(err, tc.ErrorIsNil)
	mysql := s.AddTestingApplication(c, "mysql", s.AddTestingCharm(c, "mysql"))
	...
	c.Assert(app, tc.IsNil)
}

func assertNoRelations(c *tc.C, app state.ApplicationEntity) {
	rels, err := app.Relations()
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(rels, tc.HasLen, 0)
}

func assertOneRelation(c *tc.C, srv *state.Application, relId int, endpoints ...state.Endpoint) *state.Relation {
	...
	c.Assert(rel.Endpoints(), tc.SameContents, endpoints)

	name := srv.Name()
	expectEp := endpoints[0]
	ep, err := rel.Endpoint(name)
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(ep, tc.DeepEquals, expectEp)
	if len(endpoints) == 2 {
		expectEp = endpoints[1]
	}
	eps, err := rel.RelatedEndpoints(name)
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(eps, tc.DeepEquals, []state.Endpoint{expectEp})
	return rel
}

func (s *RelationSuite) TestRemoveAlsoDeletesNetworks(c *tc.C) {
	wordpress := s.AddTestingApplication(c, "wordpress", s.AddTestingCharm(c, "wordpress"))
	wordpressEP, err := wordpress.Endpoint("db")
	c.Assert(err, tc.ErrorIsNil)
	mysql := s.AddTestingApplication(c, "mysql", s.AddTestingCharm(c, "mysql"))
...
func (s *RelationSuite) TestRemoveAlsoDeletesRemoteTokens(c *tc.C) {
	wordpress := s.AddTestingApplication(c, "wordpress", s.AddTestingCharm(c, "wordpress"))
	wordpressEP, err := wordpress.Endpoint("db")
	c.Assert(err, tc.ErrorIsNil)
	mysql := s.AddTestingApplication(c, "mysql", s.AddTestingCharm(c, "mysql"))
...
func (s *RelationSuite) TestRemoveAlsoDeletesRemoteOfferConnections(c *tc.C) {
	rwordpress, err := s.State.AddRemoteApplication(state.AddRemoteApplicationParams{
		Name:            "remote-wordpress",
		SourceModel:     names.NewModelTag("source-model"),
		IsConsumerProxy: true,
...
func (s *RelationSuite) TestRemoveAlsoDeletesSecretPermissions(c *tc.C) {
...
func (s *RelationSuite) TestRemoveNoFeatureFlag(c *tc.C) {
...
func (s *RelationSuite) TestWatchLifeSuspendedStatus(c *tc.C) {
...
func (s *RelationSuite) TestWatchLifeSuspendedStatusDead(c *tc.C) {
...
func (s *RelationSuite) setupRelationStatus(c *tc.C) *state.Relation {
	wordpress := s.AddTestingApplication(c, "wordpress", s.AddTestingCharm(c, "wordpress"))
	wordpressEP, err := wordpress.Endpoint("db")
	c.Assert(err, tc.ErrorIsNil)
	mysql := s.AddTestingApplication(c, "mysql", s.AddTestingCharm(c, "mysql"))
...
func (s *RelationSuite) TestStatus(c *tc.C) {
	rel := s.setupRelationStatus(c)
	...
	relStatus, err := rel.Status()
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(relStatus.Since, tc.NotNil)
	relStatus.Since = nil
	c.Assert(relStatus, tc.DeepEquals, status.StatusInfo{
		Status:  status.Suspended,
		Message: "for a while",
...
func (s *RelationSuite) TestInvalidStatus(c *tc.C) {
	rel := s.setupRelationStatus(c)

	err := rel.SetStatus(status.StatusInfo{
		Status: status.Status("invalid"),
	})
	c.Assert(err, tc.ErrorMatches, `cannot set invalid status "invalid"`)
}

func (s *RelationSuite) TestSetSuspend(c *tc.C) {
	rel := s.setupRelationStatus(c)
	...
	c.Assert(rel.SuspendedReason(), tc.Equals, "reason")
}

func (s *RelationSuite) TestSetSuspendFalse(c *tc.C) {
	rel := s.setupRelationStatus(c)
	// Suspend doesn't need an offer connection to be there.
	state.RemoveOfferConnectionsForRelation(c, rel)
	c.Assert(rel.Suspended(), tc.IsFalse)
	...
	c.Assert(err, tc.ErrorMatches, "cannot set suspended reason if not suspended")
...
func (s *RelationSuite) TestApplicationSettings(c *tc.C) {
	s.AddTestingApplication(c, "wordpress", s.AddTestingCharm(c, "wordpress"))
	s.AddTestingApplication(c, "mysql", s.AddTestingCharm(c, "mysql"))
	...
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(settingsMap, tc.HasLen, 0)
	...
	settingsMap, err = relation.ApplicationSettings("mysql")
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(settingsMap, tc.DeepEquals, map[string]interface{}{
		"bailterspace": "blammo",
	})
}

func (s *RelationSuite) TestApplicationSettingsPeer(c *tc.C) {
...
func (s *RelationSuite) TestApplicationSettingsErrors(c *tc.C) {
	app := state.AddTestingApplication(c, s.State, "riak", state.AddTestingCharm(c, s.State, "riak"))
	ep, err := app.Endpoint("ring")
	c.Assert(err, tc.ErrorIsNil)
	rel, err := s.State.EndpointsRelation(ep)
	...
	c.Assert(err, tc.ErrorMatches, `application "wordpress" is not a member of "riak:ring"`)
	c.Assert(settings, tc.HasLen, 0)
}

func (s *RelationSuite) TestUpdateApplicationSettingsSuccess(c *tc.C) {
	s.AddTestingApplication(c, "wordpress", s.AddTestingCharm(c, "wordpress"))
	s.AddTestingApplication(c, "mysql", s.AddTestingCharm(c, "mysql"))
	eps, err := s.State.InferEndpoints("mysql", "wordpress")
	c.Assert(err, tc.ErrorIsNil)
...
func (s *RelationSuite) TestUpdateApplicationSettingsNotLeader(c *tc.C) {
	s.AddTestingApplication(c, "wordpress", s.AddTestingCharm(c, "wordpress"))
	s.AddTestingApplication(c, "mysql", s.AddTestingCharm(c, "mysql"))
	eps, err := s.State.InferEndpoints("mysql", "wordpress")
	c.Assert(err, tc.ErrorIsNil)
...
func (s *RelationSuite) TestWatchApplicationSettings(c *tc.C) {
	s.AddTestingApplication(c, "wordpress", s.AddTestingCharm(c, "wordpress"))
	mysql := s.AddTestingApplication(c, "mysql", s.AddTestingCharm(c, "mysql"))
	eps, err := s.State.InferEndpoints("mysql", "wordpress")
	c.Assert(err, tc.ErrorIsNil)
...
func (s *RelationSuite) TestWatchApplicationSettingsOtherEnd(c *tc.C) {
	s.AddTestingApplication(c, "wordpress", s.AddTestingCharm(c, "wordpress"))
	mysql := s.AddTestingApplication(c, "mysql", s.AddTestingCharm(c, "mysql"))
	eps, err := s.State.InferEndpoints("mysql", "wordpress")
	c.Assert(err, tc.ErrorIsNil)
...
func (s *RelationSuite) TestDestroyForceSchedulesCleanupForStuckUnits(c *tc.C) {
	wordpress := s.AddTestingApplication(c, "wordpress", s.AddTestingCharm(c, "wordpress"))
	mysql := s.AddTestingApplication(c, "mysql", s.AddTestingCharm(c, "mysql"))
	eps, err := s.State.InferEndpoints("wordpress", "mysql")
	c.Assert(err, tc.ErrorIsNil)
	...
	addRelationUnit := func(c *tc.C, app *state.Application) *state.RelationUnit {
...
func (s *RelationSuite) TestDestroyForceStuckSubordinateUnits(c *tc.C) {
...
func (s *RelationSuite) TestDestroyForceStuckRemoteUnits(c *tc.C) {
...
func (s *RelationSuite) TestDestroyForceIsFineIfUnitsAlreadyLeft(c *tc.C) {
	prr := newProReqRelation(c, &s.ConnSuite, charm.ScopeGlobal)
	prr.allEnterScope(c)
	rel := prr.rel
	relUnits := []*state.RelationUnit{
...
func (s *RelationSuite) assertRelationCleanedUp(c *tc.C, rel *state.Relation, relUnits []*state.RelationUnit) {
	opErrs, err := rel.DestroyWithForce(true, time.Minute)
	c.Assert(opErrs, tc.HasLen, 0)
	c.Assert(err, tc.ErrorIsNil)
...
func (s *RelationSuite) assertNeedsCleanup(c *tc.C) {
	dirty, err := s.State.NeedsCleanup()
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(dirty, tc.IsTrue)
}

func (s *RelationSuite) assertNoCleanups(c *tc.C) {
	dirty, err := s.State.NeedsCleanup()
	c.Assert(err, tc.ErrorIsNil)
