...
func TestStatusHistorySuite(t *tctesting.T) {
	testing.MgoTestPackage(t, &StatusHistorySuite{})
}
...
func (s *StatusHistorySuite) TestPruneStatusHistoryBySize(c *tc.C) {
	...
	c.Logf("%d\n", len(history))
	c.Assert(history, tc.HasLen, initialHistory+1)
	...
	c.Assert(historyLen, tc.LessThan, expectMax)
}

func (s *StatusHistorySuite) TestPruneStatusBySizeOnlyForController(c *tc.C) {
	...
	c.Assert(err, tc.ErrorIsNil)
	c.Logf("%d\n", len(history))
	c.Assert(history, tc.HasLen, 20001)
	...
	c.Assert(historyLen, tc.Equals, 20001)
}

func (s *StatusHistorySuite) TestPruneStatusHistoryByDate(c *tc.C) {
	...
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(history, tc.HasLen, 101)
...
func (s *StatusHistorySuite) TestStatusHistoryFilterRunningUpdateStatusHook(c *tc.C) {
	application := s.Factory.MakeApplication(c, nil)
	unit := s.Factory.MakeUnit(c, &factory.UnitParams{Application: application})
	agent := unit.Agent()
	...
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(history, tc.HasLen, 200)
	stateNumber := 0
...
func (s *StatusHistorySuite) TestStatusHistoryFilterRunningUpdateStatusHookFiltered(c *tc.C) {
	application := s.Factory.MakeApplication(c, nil)
	unit := s.Factory.MakeUnit(c, &factory.UnitParams{Application: application})
	agent := unit.Agent()
...
func (s *StatusHistorySuite) TestStatusHistoryFiltersByDateAndDelta(c *tc.C) {
	...
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(history, tc.HasLen, 4)
	c.Assert(history[0].Message, tc.Equals, "current status")
	c.Assert(history[1].Message, tc.Equals, "waiting for machine")
	c.Assert(history[2].Message, tc.Equals, "2 days ago")
	c.Assert(history[3].Message, tc.Equals, "3 days ago")
...
func (s *StatusHistorySuite) TestSameValueNotRepeated(c *tc.C) {
	application := s.Factory.MakeApplication(c, nil)
	unit := s.Factory.MakeUnit(c, &factory.UnitParams{Application: application})
