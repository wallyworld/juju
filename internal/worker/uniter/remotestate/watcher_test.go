...
func TestWatcherSuiteIAAS(t *tctesting.T) {
	tc.Run(t, &WatcherSuiteIAAS{
		WatcherSuite{modelType: model.IAAS},
	})
}

func TestWatcherSuiteCAAS(t *tctesting.T) {
	tc.Run(t, &WatcherSuiteIAAS{
		WatcherSuite{modelType: model.CAAS},
	})
}

func TestWatcherSuiteSidecar(t *tctesting.T) {
...
func TestWatcherSuiteSidecarCharmModVer(t *tctesting.T) {
...
func (s *WatcherSuiteCAAS) SetUpTest(c *tc.C) {
	s.WatcherSuite.SetUpTest(c)
...
func (s *WatcherSuiteCAAS) TestInitialSnapshot(c *tc.C) {
	snap := s.watcher.Snapshot()
	c.Assert(snap, tc.DeepEquals, remotestate.Snapshot{
		Relations:               map[int]remotestate.RelationSnapshot{},
		Storage:                 map[names.StorageTag]remotestate.StorageSnapshot{},
...
func (s *WatcherSuiteCAAS) TestSnapshot(c *tc.C) {
	s.signalAll()
	assertNotifyEvent(c, s.watcher.RemoteStateChanged(), "waiting for remote state change")
...
func (s *WatcherSuite) TestRemoteStateChanged(c *tc.C) {
	...
	s.st.unit.application.leaderSettingsWatcher.changes <- struct{}{}
	assertOneChange()
	c.Assert(s.watcher.Snapshot().LeaderSettingsVersion, tc.Equals, initial.LeaderSettingsVersion+1)
...
func (s *WatcherSuite) TestRelationsChanged(c *tc.C) {
	...
	c.Assert(s.st.relationUnitsWatchers[relationTag].Stopped(), tc.IsTrue)
}
