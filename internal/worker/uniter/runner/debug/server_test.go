...
func TestDebugHooksServerSuite(t *tctesting.T) {
...
func (s *DebugHooksServerSuite) TestFindSession(c *tc.C) {
	...
	c.Assert(session.MatchHook(""), tc.IsTrue)
	c.Assert(session.MatchHook("something"), tc.IsTrue)
	...
	c.Assert(session.MatchHook(""), tc.IsFalse)
	c.Assert(session.MatchHook("something"), tc.IsFalse)
	c.Assert(session.MatchHook("foo"), tc.IsTrue)
	c.Assert(session.MatchHook("bar"), tc.IsTrue)
	c.Assert(session.MatchHook("baz"), tc.IsTrue)
	c.Assert(session.MatchHook("foo bar baz"), tc.IsFalse)
...
func (s *DebugHooksServerSuite) TestRunHookExceptional(c *tc.C) {
	...
	err = session.RunHook("myhook", s.tmpdir, os.Environ(), "myhook")
	waitForFlock()
	c.Assert(clientExited, tc.IsTrue)
	c.Assert(err, tc.ErrorMatches, "signal: [kK]illed")
}
...
func (s *DebugHooksServerSuite) TestRunHook(c *tc.C) {
	...
	c.Assert(entries[0].IsDir(), tc.IsTrue)
	c.Assert(strings.HasPrefix(entries[0].Name(), "juju-debug-hooks-"), tc.IsTrue)
...
func (s *DebugHooksServerSuite) verifyEnvshFile(c *tc.C, envshPath string, hookName string) {
	...
	c.Assert(contents, tc.Contains, fmt.Sprintf("JUJU_UNIT_NAME=%q", s.ctx.Unit))
	c.Assert(contents, tc.Contains, fmt.Sprintf(`PS1="%s:hooks/%s %% "`, s.ctx.Unit, hookName))
}
...
func TestDebugSuite(t *tctesting.T) {
	tc.Run(t, &DebugSuite{})
}
