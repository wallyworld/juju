...
func (s *manifoldSuite) TestStart(c *tc.C) {
	...
	c.Assert(args[2], tc.NotNil)
}

func (s *manifoldSuite) TestStartWithAuditingDisabled(c *tc.C) {
	...
	auditConfig := args[1].(auditlog.Config)
	c.Assert(auditConfig.Target, tc.IsNil)
}
...
func (s *manifoldSuite) TestOutput(c *tc.C) {
	...
	c.Assert(auditConfig, tc.DeepEquals, getConfig())
}
...
func (s *manifoldSuite) TestClosesStateOnWorkerError(c *tc.C) {
