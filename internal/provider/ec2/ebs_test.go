...
func TestEbsSuite(t *tctesting.T) {
	tc.Run(t, &ebsSuite{})
}
...
func (s *ebsSuite) TestDestroyVolumesCredentialError(c *tc.C) {
	...
	for i, result := range results {
		...
		c.Assert(errors.Is(result, common.ErrorCredentialNotValid), tc.IsTrue)
	}
}
...
func (s *ebsSuite) TestAttachVolumesCredentialError(c *tc.C) {
	...
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(results, tc.HasLen, 1)
	c.Assert(errors.Is(results[0].Error, common.ErrorCredentialNotValid), tc.IsTrue)
}
...
func (s *ebsSuite) TestCreateVolumesCredentialError(c *tc.C) {
	...
	for i, result := range results {
		...
		c.Assert(errors.Is(result.Error, common.ErrorCredentialNotValid), tc.IsTrue)
	}
}
...
func TestBlockDeviceMappingSuite(t *tctesting.T) {
