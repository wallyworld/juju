...
func overrideCinderProvider(s *testhelpers.CleanupSuite, adapter *mockAdapter) {
...
func assertInstanceIds(c *tc.C, env environs.Environ, callCtx context.ProviderCallContext, expected ...instance.Id) {
...
func (s *localServerSuite) TestInstanceStatus(c *tc.C) {
	...
	inst, _ := testing.AssertStartInstance(c, s.env, s.callCtx, s.ControllerUUID, "100")
	c.Assert(inst.Status(s.callCtx).Status, tc.Equals, status.Running)
	err := s.env.StopInstances(s.callCtx, inst.Id())
	c.Assert(err, tc.ErrorIsNil)
}
...
func (s *localServerSuite) assertInstancesGathering(c *tc.C, withFloatingIP bool) {
	...
	for i, test := range instanceGathering {
		...
		c.Assert(err, tc.Equals, test.err)
...
func (s *localServerSuite) TestInstancesShutoffSuspended(c *tc.C) {
	...
	c.Assert(twoInstances[0].Status(s.callCtx).Message, tc.Equals, nova.StatusShutoff)
	c.Assert(twoInstances[1].Status(s.callCtx).Message, tc.Equals, nova.StatusSuspended)
}
...
func (s *localServerSuite) TestSuperSubnets(c *tc.C) {
	env := s.prepareNetworkingEnviron(c, s.env.Config())
	...
	c.Check(obtainedSubnets, tc.DeepEquals, expectedSubnets)
}
...
func TestLocalHTTPSServerSuite(t *tctesting.T) {
...
func addVolume(
	c *tc.C, env environs.Environ, callCtx context.ProviderCallContext, controllerUUID, name string,
) *storage.Volume {
...
func TestNoSwiftSuite(t *tctesting.T) {
