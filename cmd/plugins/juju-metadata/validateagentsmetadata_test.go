...
func TestValidateToolsMetadataSuite(t *tctesting.T) {
...
func (s *ValidateToolsMetadataSuite) TestEc2LocalMetadataUsingEnvironment(c *tc.C) {
	...
	c.Assert(strippedOut, tc.Matches, `Matching Tools Versions:.*Resolve Metadata.*`)
}
...
func (s *ValidateToolsMetadataSuite) TestEc2LocalMetadataWithManualParams(c *tc.C) {
	...
	errOut := cmdtesting.Stdout(ctx)
	strippedOut := strings.Replace(errOut, "\n", "", -1)
	c.Check(strippedOut, tc.Matches, `Matching Tools Versions:.*Resolve Metadata.*`)
}
