...
func TestRunTestSuite(t *tctesting.T) {
...
func (*RunTestSuite) TestArgParsing(c *tc.C) {
	for i, test := range []struct {
		...
		if test.errMatch == "" {
			...
			c.Assert(runCommand.operator, tc.Equals, test.operator)
		} else {
...
func (s *RunTestSuite) TestMissingSocket(c *tc.C) {
	...
	c.Assert(err, tc.ErrorMatches, `.*dial unix .*/run.socket:.*`+utils.NoSuchFileErrRegexp)
}
...
func (s *RunTestSuite) TestRunning(c *tc.C) {
	...
	c.Check(cmd.IsRcPassthroughError(err), tc.IsTrue)
...
func (s *RunTestSuite) TestRunningBadRelation(c *tc.C) {
	...
	c.Check(cmd.IsRcPassthroughError(err), tc.IsFalse)
