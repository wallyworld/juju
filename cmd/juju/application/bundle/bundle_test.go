...
func TestBuildModelRepSuite(t *tctesting.T) {
...
func TestComposeAndVerifyRepSuite(t *tctesting.T) {
...
func (s *composeAndVerifyRepSuite) TestComposeAndVerifyBundleMixingBaseAndSeries(c *tc.C) {
	defer s.setupMocks(c).Finish()
...
func (s *composeAndVerifyRepSuite) TestComposeAndVerifyBundleMixingBaseAndSeriesMisMatch(c *tc.C) {
	defer s.setupMocks(c).Finish()
	...
	obtained, _, err := ComposeAndVerifyBundle(ctx, s.bundleDataSource, []string{s.overlayFile})
	c.Assert(err, tc.ErrorMatches, `(?s)the provided bundle has the following errors:.*application "wordpress" series "jammy" and base "ubuntu@20.04" must match if both supplied.*invalid constraints.*`)
	c.Assert(obtained, tc.IsNil)
}
...
func (m stringSliceMatcher) Matches(x interface{}) bool {
	...
	m.c.Assert(obtained, tc.SameContents, m.expected)
	return true
}
