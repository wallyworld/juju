...
func TestAddSuite(t *tctesting.T) {
	tc.Run(t, &addSuite{})
}
...
func (s *addSuite) TestInvalidCredentialMessage(c *tc.C) {
	fake := newFakeCloudMetadataStore()
	fake.Call("PublicCloudMetadata", []string(nil)).Returns(map[string]jujucloud.Cloud{}, false, nil)
	fake.Call("PersonalCloudMetadata").Returns(map[string]jujucloud.Cloud{}, nil)
	const expectedYAMLarg = "" +
	...
	err = command.Run(ctx)
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(cmdtesting.Stderr(ctx), tc.Contains, "Cloud credential is not accepted by cloud provider: running test")
}
...
func TestAddOpenStackSuite(t *tctesting.T) {
