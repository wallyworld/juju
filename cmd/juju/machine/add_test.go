...
func TestAddMachineSuite(t *tctesting.T) {
...
func (s *AddMachineSuite) TestSeriesAndBaseError(c *tc.C) {
	_, err := s.run(c, "--series=jammy", "--base=ubuntu@22.04")
	c.Assert(err, tc.ErrorMatches, "--series and --base cannot be specified together")
}
...
func (s *AddMachineSuite) TestAddMachineWithDisks(c *tc.C) {
	...
	c.Assert(s.fakeAddMachine.args, tc.HasLen, 1)
	param := s.fakeAddMachine.args[0]
	c.Assert(param.Disks, tc.DeepEquals, []storage.Constraints{
		{Size: 1024, Count: 2},
		{Size: 2048, Count: 1},
	})
}
