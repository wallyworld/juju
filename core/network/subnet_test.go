...
func (*subnetSuite) TestFindSubnetIDsForAZ(c *tc.C) {
	...
	for i, t := range testCases {
		...
		if t.expectedErr != nil {
			c.Check(t.expectedErr(err), tc.IsTrue)
		} else {
...
func (*subnetSuite) TestSubnetInfosGetByUnderLayCIDR(c *tc.C) {
	s := network.SubnetInfos{
		{
			ID:      "1",
	...
	c.Check(overlays, tc.DeepEquals, network.SubnetInfos{s[0]})

	overlays, err = s.GetByUnderlayCIDR(s[1].FanLocalUnderlay())
	c.Assert(err, tc.ErrorIsNil)
	c.Check(overlays, tc.DeepEquals, network.SubnetInfos{s[1], s[2]})

	overlays, err = s.GetByUnderlayCIDR("30.30.30.0/24")
	c.Assert(err, tc.ErrorIsNil)
	c.Check(overlays, tc.HasLen, 0)
}
...
func (*subnetSuite) TestSubnetInfosGetBySpaceID(c *tc.C) {
	s := network.SubnetInfos{
		{
			ID:      "1",
			CIDR:    "10.10.10.0/24",
	...
	c.Assert(err, tc.ErrorIsNil)
	c.Check(subs, tc.DeepEquals, network.SubnetInfos{
		{
			ID:      "1",
			CIDR:    "10.10.10.0/24",
			SpaceID: "666",
	...
	c.Assert(err, tc.ErrorIsNil)
	c.Check(subs, tc.DeepEquals, s[2:])
}
