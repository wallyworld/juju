...
func TestSpaceSuite(t *tctesting.T) {
	tc.Run(t, &spaceSuite{})
}
...
func (s *spaceSuite) TestFanOverlaysFor(c *tc.C) {
	...
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(overlays, tc.HasLen, 0)

	overlays, err = s.spaces.FanOverlaysFor(network.MakeIDSet("14"))
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(overlays, tc.DeepEquals, network.SubnetInfos{overlay})
}
