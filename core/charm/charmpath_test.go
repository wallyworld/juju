...
func TestCharmPathSuite(t *tctesting.T) {
...
func (s *charmPathSuite) TestNoPath(c *tc.C) {
	...
	c.Assert(err, tc.ErrorMatches, "empty charm path")
}
...
func (s *charmPathSuite) TestNoCharmAtPath(c *tc.C) {
	...
	c.Assert(err, tc.ErrorMatches, "charm not found.*")
}

func (s *charmPathSuite) TestCharm(c *tc.C) {
...
func (s *charmPathSuite) TestFindsSymlinks(c *tc.C) {
	...
	c.Assert(ch.(*charm.CharmDir).Path, tc.Equals, linkPath)
	c.Assert(url, tc.DeepEquals, charm.MustParseURL("local:dummy-1"))
}
