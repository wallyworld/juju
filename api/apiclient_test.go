...
func TestApiclientSuite(t *tctesting.T) {
	jtesting.MgoTestPackage(t, &apiclientSuite{})
}
...
func (s *apiclientSuite) TestOpen(c *tc.C) {
	...
	c.Assert(st.Addr().String(), tc.Equals, "wss://"+info.Addrs[0])
	modelTag, ok := st.ModelTag()
	c.Assert(ok, tc.IsTrue)
	c.Assert(modelTag, tc.Equals, s.Model.ModelTag())
	...
	c.Assert(api.CookieURL(st).String(), tc.Equals, "https://deadbeef-1bad-500d-9000-4b1d0d06f00d/")
}
...
func (s *apiclientSuite) TestOpenCookieURLDefaultsToAddress(c *tc.C) {
	...
	c.Assert(api.CookieURL(st).String(), tc.Matches, "https://localhost:.*/")
}
...
func (s *apiclientSuite) TestOpenWithNoCACert(c *tc.C) {
	...
	c.Assert(err, tc.ErrorMatches, `unable to connect to API:.*x509: certificate signed by unknown authority`)
...
func (s *apiclientSuite) TestIsBrokenOk(c *tc.C) {
	...
	c.Assert(conn.IsBroken(), tc.IsFalse)
}
...
func (s *apiclientSuite) TestIsBrokenChannelClosed(c *tc.C) {
	...
	c.Assert(conn.IsBroken(), tc.IsTrue)
}
...
func (s *apiclientSuite) TestWatchDebugLogConnected(c *tc.C) {
	cl := apiclient.NewClient(s.APIState, loggertesting.WrapCheckLog(c))
...
func (s *apiclientSuite) TestConnectStreamAtUUIDPath(c *tc.C) {
	...
	connectURL, err := url.Parse(catcher.Location())
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(connectURL.Path, tc.Matches, fmt.Sprintf("/model/%s/path", model.UUID()))
}
...
func TestClientDNSNameSuite(t *tctesting.T) {
	jtesting.MgoTestPackage(t, &clientDNSNameSuite{})
}

func (s *clientDNSNameSuite) SetUpTest(c *tc.C) {
...
func (s *clientDNSNameSuite) TestPublicDNSName(c *tc.C) {
	apiInfo := s.APIInfo(c)
