...
func (s *provisionerSuite) assertGetOneMachine(c *tc.C, tag names.MachineTag) provisioner.MachineProvisioner {
	result, err := s.provisioner.Machines(tag)
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(len(result), tc.Equals, 1)
	c.Assert(result[0].Err, tc.IsNil)
	return result[0].Machine
}

func (s *provisionerSuite) TestMachinesTagAndId(c *tc.C) {
	result, err := s.provisioner.Machines(names.NewMachineTag("42"), s.machine.MachineTag())
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(len(result), tc.Equals, 2)

	c.Assert(result[0].Err, tc.ErrorMatches, "machine 42 not found")
	c.Assert(result[0].Err, tc.Satisfies, params.IsCodeNotFound)
	c.Assert(result[0].Machine, tc.IsNil)

	c.Assert(result[1].Err, tc.IsNil)
	c.Assert(result[1].Machine.Tag(), tc.Equals, s.machine.Tag())
	c.Assert(result[1].Machine.Id(), tc.Equals, s.machine.Id())
}

func (s *provisionerSuite) TestGetSetStatus(c *tc.C) {
	apiMachine := s.assertGetOneMachine(c, s.machine.MachineTag())

	machineStatus, info, err := apiMachine.Status()
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(machineStatus, tc.Equals, status.Pending)
	c.Assert(info, tc.Equals, "")
	...
	machineStatus, info, err = apiMachine.Status()
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(machineStatus, tc.Equals, status.Started)
	c.Assert(info, tc.Equals, "blah")
	statusInfo, err := s.machine.Status()
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(statusInfo.Data, tc.HasLen, 0)
}

func (s *provisionerSuite) TestGetSetInstanceStatus(c *tc.C) {
	apiMachine := s.assertGetOneMachine(c, s.machine.MachineTag())
	instanceStatus, info, err := apiMachine.InstanceStatus()
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(instanceStatus, tc.Equals, status.Pending)
	c.Assert(info, tc.Equals, "")
	err = apiMachine.SetInstanceStatus(status.Running, "blah", nil)
	c.Assert(err, tc.ErrorIsNil)
	instanceStatus, info, err = apiMachine.InstanceStatus()
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(instanceStatus, tc.Equals, status.Running)
	c.Assert(info, tc.Equals, "blah")
...
func (s *provisionerSuite) TestGetSetStatusWithData(c *tc.C) {
	apiMachine := s.assertGetOneMachine(c, s.machine.MachineTag())
	...
	machineStatus, info, err := apiMachine.Status()
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(machineStatus, tc.Equals, status.Error)
	c.Assert(info, tc.Equals, "blah")
	statusInfo, err := s.machine.Status()
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(statusInfo.Data, tc.DeepEquals, map[string]interface{}{"foo": "bar"})
}
...
func (s *provisionerSuite) TestMachinesWithTransientErrors(c *tc.C) {
	...
	c.Assert(result[0].Machine.Id(), tc.Equals, "1")
...
func (s *provisionerSuite) TestEnsureDeadAndRemove(c *tc.C) {
	...
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(otherMachine.Life(), tc.Equals, state.Alive)

	apiMachine := s.assertGetOneMachine(c, otherMachine.MachineTag())
	c.Assert(err, tc.ErrorIsNil)

	err = apiMachine.Remove()
	c.Assert(err, tc.ErrorMatches, `cannot remove entity "machine-1": still alive`)
	err = apiMachine.EnsureDead()
	c.Assert(err, tc.ErrorIsNil)

	err = otherMachine.Refresh()
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(otherMachine.Life(), tc.Equals, state.Dead)
	...
	apiMachine = s.assertGetOneMachine(c, s.machine.MachineTag())
	err = apiMachine.EnsureDead()
	c.Assert(err, tc.ErrorMatches, "machine 0 is still a non-voting controller member")
}
...
func (s *provisionerSuite) TestMarkForRemoval(c *tc.C) {
	...
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(removals, tc.SameContents, []string{"1"})
}

func (s *provisionerSuite) TestRefreshAndLife(c *tc.C) {
	// Create a fresh machine to test the complete scenario.
	otherMachine, err := s.State.AddMachine(state.UbuntuBase("12.10"), state.JobHostUnits)
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(otherMachine.Life(), tc.Equals, state.Alive)
	...
	apiMachine := s.assertGetOneMachine(c, otherMachine.MachineTag())
	c.Assert(apiMachine.Life(), tc.Equals, life.Alive)
	...
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(apiMachine.Life(), tc.Equals, life.Dead)
}
...
func (s *provisionerSuite) TestSetInstanceInfo(c *tc.C) {
	...
	c.Assert(err, tc.ErrorMatches, "machine 1 not provisioned")
	c.Assert(instanceId, tc.Equals, instance.Id(""))
	...
	instanceId, err = apiMachine.InstanceId()
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(instanceId, tc.Equals, instance.Id("i-will"))

	// Try it again - should fail.
	err = apiMachine.SetInstanceInfo("i-wont", "", "fake", nil, nil, nil, nil, nil)
	c.Assert(err, tc.ErrorMatches, `cannot record provisioning info for "i-wont": cannot set instance data for machine "1": already set`)
	...
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(stateVolumeAttachments, tc.HasLen, 1)
...
func (s *provisionerSuite) TestAvailabilityZone(c *tc.C) {
	...
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(availabilityZone, tc.Equals, retAvailabilityZone)
}

func (s *provisionerSuite) TestSetInstanceInfoProfiles(c *tc.C) {
	// Create a fresh machine, since machine 0 is already provisioned.
	template := state.MachineTemplate{
		Base: state.UbuntuBase("16.04"),
		Jobs: []state.MachineJob{state.JobHostUnits},
	...
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(profiles, tc.SameContents, obtainedProfiles)
}
...
func (s *provisionerSuite) TestDistributionGroup(c *tc.C) {
	...
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(instances, tc.DeepEquals, []instance.Id{"i-manager"})
	...
	instances, err = apiMachine.DistributionGroup()
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(instances, tc.HasLen, 0) // no units assigned
	...
	for i := 0; i < 3; i++ {
		...
		instances, err := apiMachine.DistributionGroup()
		c.Assert(err, tc.ErrorIsNil)
		c.Assert(instances, tc.DeepEquals, []instance.Id{"i-d"})
	}
}

func (s *provisionerSuite) TestDistributionGroupMachineNotFound(c *tc.C) {
...
func (s *provisionerSuite) TestDistributionGroupByMachineId(c *tc.C) {
	...
	c.Assert(results, tc.DeepEquals, []provisioner.DistributionGroupResult{
	...
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(len(results), tc.Equals, 3)
	c.Assert(results, tc.DeepEquals, []provisioner.DistributionGroupResult{
		{MachineIds: nil, Err: nil},
...
func (s *provisionerSuite) TestDistributionGroupByMachineIdNotFound(c *tc.C) {
	stateMachine, err := s.State.AddMachine(state.UbuntuBase("12.10"), state.JobHostUnits)
	c.Assert(err, tc.ErrorIsNil)
	...
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(len(results), tc.Equals, 1)
	c.Assert(results[0].Err, tc.ErrorMatches, "machine 1 not found")
	c.Assert(results[0].Err, tc.Satisfies, params.IsCodeNotFound)
}
...
func (s *provisionerSuite) TestProvisioningInfo(c *tc.C) {
	...
	c.Assert(provisioningInfo.Base, tc.DeepEquals, params.Base{Name: "ubuntu", Channel: "12.10/stable"})
	c.Assert(provisioningInfo.Placement, tc.Equals, template.Placement)
	c.Assert(provisioningInfo.Constraints, tc.DeepEquals, template.Constraints)

	c.Assert(provisioningInfo.SubnetAZs, tc.DeepEquals, map[string][]string{
		"subnet-2": {"zone2"},
		"subnet-3": {"zone3"},
	})

	c.Assert(provisioningInfo.SpaceSubnets, tc.HasLen, 1)
	c.Assert(provisioningInfo.SpaceSubnets["space2"], tc.SameContents, []string{"subnet-2", "subnet-3"})
}

func (s *provisionerSuite) TestProvisioningInfoMachineNotFound(c *tc.C) {
	res, err := s.provisioner.ProvisioningInfo([]names.MachineTag{names.NewMachineTag("1")})
	c.Assert(err, tc.ErrorIsNil)

	results := res.Results
	c.Assert(results, tc.HasLen, 1)
	c.Assert(results[0].Error, tc.ErrorMatches, "machine 1 not found")
	c.Assert(results[0].Error, tc.Satisfies, params.IsCodeNotFound)
}
...
func (s *provisionerSuite) TestWatchContainersAcceptsSupportedContainers(c *tc.C) {
	apiMachine := s.assertGetOneMachine(c, s.machine.MachineTag())
...
func (s *provisionerSuite) TestWatchContainersErrors(c *tc.C) {
	apiMachine := s.assertGetOneMachine(c, s.machine.MachineTag())

	_, err := apiMachine.WatchContainers(instance.NONE)
	c.Assert(err, tc.ErrorMatches, `unsupported container type "none"`)

	_, err = apiMachine.WatchContainers("")
	c.Assert(err, tc.ErrorMatches, "container type must be specified")
}
...
func (s *provisionerSuite) TestContainerManagerConfigKVM(c *tc.C) {
...
func (s *provisionerSuite) TestContainerManagerConfigPermissive(c *tc.C) {
...
func (s *provisionerSuite) TestContainerConfig(c *tc.C) {
	...
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(result.ProviderType, tc.Equals, "dummy")
	c.Assert(result.AuthorizedKeys, tc.Equals, s.Environ.Config().AuthorizedKeys())
	c.Assert(result.SSLHostnameVerification, tc.IsTrue)
}
...
func (s *provisionerSuite) TestSupportsNoContainers(c *tc.C) {
	apiMachine := s.assertGetOneMachine(c, s.machine.MachineTag())
...
func (s *provisionerSuite) TestFindToolsNoArch(c *tc.C) {
	s.testFindTools(c, false, nil, nil)
}

func (s *provisionerSuite) TestFindToolsArch(c *tc.C) {
	s.testFindTools(c, true, nil, nil)
}

func (s *provisionerSuite) TestFindToolsAPIError(c *tc.C) {
	apiError := errors.New("everything's broken")
	s.testFindTools(c, false, apiError, nil)
}

func (s *provisionerSuite) TestFindToolsLogicError(c *tc.C) {
	logicError := errors.NotFoundf("tools")
	s.testFindTools(c, false, nil, logicError)
}

func (s *provisionerSuite) testFindTools(c *tc.C, matchArch bool, apiError, logicError error) {
	...
	provisioner.PatchFacadeCall(s, s.provisioner, func(request string, args, response interface{}) error {
		called = true
		c.Assert(request, tc.Equals, "FindTools")
		expected := params.FindToolsParams{
			Number: jujuversion.Current,
			OSType: "ubuntu",
			Arch:   a,
		}
		c.Assert(args, tc.Equals, expected)
	...
	if apiError != nil {
		c.Assert(err, tc.Equals, apiError)
	} else if logicError != nil {
		c.Assert(err.Error(), tc.Equals, logicError.Error())
	} else {
		c.Assert(err, tc.ErrorIsNil)
		c.Assert(apiList, tc.SameContents, toolsList)
	}
}
...
func (s *provisionerSuite) TestHostChangesForContainer(c *tc.C) {
	...
	c.Assert(err, tc.ErrorIsNil)
	c.Check(changes, tc.DeepEquals, []network.DeviceToBridge{{
		BridgeName: "br-ens3",
		DeviceName: "ens3",
	}})
...
func TestProvisionerContainerSuite(t *tctesting.T) {
...
func (s *provisionerContainerSuite) SetUpTest(c *tc.C) {
