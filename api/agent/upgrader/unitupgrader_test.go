// Copyright 2012, 2013 Canonical Ltd.
// Licensed under the AGPLv3, see LICENCE file for details.

package upgrader_test

import (
	tctesting "testing"

	"github.com/juju/errors"
	"github.com/juju/tc"
	"github.com/juju/utils/v3"
	"github.com/juju/version/v2"

	"github.com/juju/juju/api"
	"github.com/juju/juju/api/agent/upgrader"
	"github.com/juju/juju/core/arch"
	"github.com/juju/juju/core/instance"
	"github.com/juju/juju/core/watcher/watchertest"
	"github.com/juju/juju/internal/testing"
	jujutesting "github.com/juju/juju/juju/testing"
	"github.com/juju/juju/rpc/params"
	"github.com/juju/juju/state"
	"github.com/juju/juju/tools"
)

type unitUpgraderSuite struct {
	jujutesting.JujuConnSuite

	stateAPI api.Connection

	// These are raw State objects. Use them for setup and assertions, but
	// should never be touched by the API calls themselves
	rawMachine *state.Machine
	rawUnit    *state.Unit

	st *upgrader.State
}

func TestUnitUpgraderSuite(t *tctesting.T) {
	testing.MgoTestPackage(t, &unitUpgraderSuite{})
}

func (s *unitUpgraderSuite) SetUpTest(c *tc.C) {
	s.JujuConnSuite.SetUpTest(c)

	s.rawMachine, _, _, s.rawUnit = s.addMachineApplicationCharmAndUnit(c, "wordpress")
	password, err := utils.RandomPassword()
	c.Assert(err, tc.ErrorIsNil)
	err = s.rawUnit.SetPassword(password)
	c.Assert(err, tc.ErrorIsNil)
	s.stateAPI = s.OpenAPIAs(c, s.rawUnit.Tag(), password)

	// Create the upgrader facade.
	s.st = upgrader.NewState(s.stateAPI)
	c.Assert(s.st, tc.NotNil)

	s.WaitForModelWatchersIdle(c, s.State.ModelUUID())
}

func (s *unitUpgraderSuite) addMachineApplicationCharmAndUnit(c *tc.C, appName string) (*state.Machine, *state.Application, *state.Charm, *state.Unit) {
	machine, err := s.State.AddMachine(state.UbuntuBase("12.10"), state.JobHostUnits)
	c.Assert(err, tc.ErrorIsNil)

	arch := arch.DefaultArchitecture
	hwChar := &instance.HardwareCharacteristics{
		Arch: &arch,
	}
	instId := instance.Id("i-host-machine")
	err = machine.SetProvisioned(instId, "", "fake-nonce", hwChar)
	c.Assert(err, tc.ErrorIsNil)

	charm := s.AddTestingCharm(c, appName)
	app := s.AddTestingApplication(c, appName, charm)
	unit, err := app.AddUnit(state.AddUnitParams{})
	c.Assert(err, tc.ErrorIsNil)
	err = unit.AssignToMachine(machine)
	c.Assert(err, tc.ErrorIsNil)
	return machine, app, charm, unit
}

func (s *unitUpgraderSuite) TestSetVersionWrongUnit(c *tc.C) {
	err := s.st.SetVersion("unit-wordpress-42", testing.CurrentVersion())
	c.Assert(err, tc.ErrorMatches, "permission denied")
	c.Assert(err, tc.Satisfies, params.IsCodeUnauthorized)
}

func (s *unitUpgraderSuite) TestSetVersionNotUnit(c *tc.C) {
	err := s.st.SetVersion("foo-42", testing.CurrentVersion())
	c.Assert(err, tc.ErrorMatches, "permission denied")
	c.Assert(err, tc.Satisfies, params.IsCodeUnauthorized)
}

func (s *unitUpgraderSuite) TestSetVersion(c *tc.C) {
	current := testing.CurrentVersion()
	agentTools, err := s.rawUnit.AgentTools()
	c.Assert(err, tc.Satisfies, errors.IsNotFound)
	c.Assert(agentTools, tc.IsNil)
	err = s.st.SetVersion(s.rawUnit.Tag().String(), current)
	c.Assert(err, tc.ErrorIsNil)
	s.rawUnit.Refresh()
	agentTools, err = s.rawUnit.AgentTools()
	c.Assert(err, tc.ErrorIsNil)
	c.Check(agentTools.Version, tc.Equals, current)
}

func (s *unitUpgraderSuite) TestToolsWrongUnit(c *tc.C) {
	tools, err := s.st.Tools("unit-wordpress-42")
	c.Assert(err, tc.ErrorMatches, "permission denied")
	c.Assert(err, tc.Satisfies, params.IsCodeUnauthorized)
	c.Assert(tools, tc.IsNil)
}

func (s *unitUpgraderSuite) TestToolsNotUnit(c *tc.C) {
	tools, err := s.st.Tools("foo-42")
	c.Assert(err, tc.ErrorMatches, "permission denied")
	c.Assert(err, tc.Satisfies, params.IsCodeUnauthorized)
	c.Assert(tools, tc.IsNil)
}

func (s *unitUpgraderSuite) TestTools(c *tc.C) {
	current := testing.CurrentVersion()
	curTools := &tools.Tools{Version: current, URL: ""}
	curTools.Version.Minor++
	s.rawMachine.SetAgentVersion(current)
	// UnitUpgrader.Tools returns the *desired* set of tools, not the currently
	// running set. We want to be upgraded to cur.Version
	stateToolsList, err := s.st.Tools(s.rawUnit.Tag().String())
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(stateToolsList, tc.HasLen, 1)
	stateTools := stateToolsList[0]
	c.Check(stateTools.Version.Number, tc.DeepEquals, current.Number)
	c.Assert(stateTools.URL, tc.NotNil)
}

func (s *unitUpgraderSuite) TestWatchAPIVersion(c *tc.C) {
	w, err := s.st.WatchAPIVersion(s.rawUnit.Tag().String())
	c.Assert(err, tc.ErrorIsNil)
	wc := watchertest.NewNotifyWatcherC(c, w)
	defer wc.AssertStops()

	// Initial event
	wc.AssertOneChange()
	vers := version.MustParseBinary("10.20.34-ubuntu-amd64")
	err = s.rawMachine.SetAgentVersion(vers)
	c.Assert(err, tc.ErrorIsNil)

	// One change noticing the new version
	wc.AssertOneChange()
	vers = version.MustParseBinary("10.20.35-ubuntu-amd64")
	err = s.rawMachine.SetAgentVersion(vers)
	c.Assert(err, tc.ErrorIsNil)
	wc.AssertOneChange()
}

func (s *unitUpgraderSuite) TestWatchAPIVersionWrongUnit(c *tc.C) {
	_, err := s.st.WatchAPIVersion("unit-wordpress-42")
	c.Assert(err, tc.ErrorMatches, "permission denied")
	c.Assert(err, tc.Satisfies, params.IsCodeUnauthorized)
}

func (s *unitUpgraderSuite) TestWatchAPIVersionNotUnit(c *tc.C) {
	_, err := s.st.WatchAPIVersion("foo-42")
	c.Assert(err, tc.ErrorMatches, "permission denied")
	c.Assert(err, tc.Satisfies, params.IsCodeUnauthorized)
}

func (s *unitUpgraderSuite) TestDesiredVersion(c *tc.C) {
	current := testing.CurrentVersion()
	curTools := &tools.Tools{Version: current, URL: ""}
	curTools.Version.Minor++
	s.rawMachine.SetAgentVersion(current)
	// UnitUpgrader.DesiredVersion returns the *desired* set of tools, not the
	// currently running set. We want to be upgraded to cur.Version
	stateVersion, err := s.st.DesiredVersion(s.rawUnit.Tag().String())
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(stateVersion, tc.Equals, current.Number)
}

func (s *unitUpgraderSuite) TestDesiredVersionWrongUnit(c *tc.C) {
	_, err := s.st.DesiredVersion("unit-wordpress-42")
	c.Assert(err, tc.ErrorMatches, "permission denied")
	c.Assert(err, tc.Satisfies, params.IsCodeUnauthorized)
}

func (s *unitUpgraderSuite) TestDesiredVersionNotUnit(c *tc.C) {
	_, err := s.st.DesiredVersion("foo-42")
	c.Assert(err, tc.ErrorMatches, "permission denied")
	c.Assert(err, tc.Satisfies, params.IsCodeUnauthorized)
}
